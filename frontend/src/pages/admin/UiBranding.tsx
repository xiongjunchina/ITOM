...
export default function UiBranding() {
  ...
  return <Card loading={loading} title="界面与品牌" extra={<Space><Button onClick={() => void reset()}>恢复默认</Button><Button icon={<SaveOutlined />} onClick={() => void save()}>保存草稿</Button><Button onClick={() => void preview()}>预览草稿</Button><Button type="primary" onClick={() => void publish()}>发布</Button></Space>}>
    ...
    <Form form={form} layout="vertical"><Tabs items={[
      ...
      {key:'legal',label:'开发者与法律',children:<><Alert showIcon type="info" message="这里维护可展示的开发者、厂商与法律信息；软件版本号由 Git 发布清单只读生成，不能在此修改。" style={{marginBottom:16}}/><Row gutter={24}><Col span={12}>{field('legal','developer_name_zh','开发者（中文）')}{field('legal','developer_name_en','Developer')}{field('legal','vendor_name_zh','厂商（中文）')}{field('legal','vendor_name_en','Vendor')}{field('legal','website_url','官方网站')}{field('legal','support_url','技术支持地址')}</Col><Col span={12}>{field('legal','copyright_holder_zh','版权主体（中文）')}{field('legal','copyright_holder_en','Copyright holder')}{field('legal','copyright_year','版权年份')}{field('legal','license_name_zh','许可名称（中文）')}{field('legal','license_name_en','License name')}{field('legal','license_url','软件许可地址')}{field('legal','third_party_notices_url','第三方开源声明地址')}</Col></Row></>},
