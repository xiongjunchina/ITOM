...
entity_type、entity_id、filename、storage_path、size、uploaded_by。合同附件、项目文档、章程原件、Bug 证据和服务请求补充附件共用；Bug 固定使用 `entity_type=bug`、`entity_id=bug.id`，服务请求正式文件固定使用 `entity_type=ticket`、`entity_id=ticket.id`，均不新增专用附件表。服务请求建单前临时文件使用 `entity_type=ticket_draft`、`entity_id=auth_user.id`，只允许上传人随后在同一建单事务绑定或取消；未绑定草稿 24 小时后软删并清理物理文件，不能通过通用清单或下载读取。证据说明文本仍保留在 `bug.evidence`。
