...
`requirement_id` FK、`name`、`description`、`assignee` FK、`plan_date`、`plan_effort`、`actual_effort`、`status`（待处理/进行中/已完成）、`done_at`[计]。同一需求允许多行任务，任务记录继续使用 `GlidBase.is_deleted` 软删除。开发任务模板导入只将每行映射到这些既有字段，不新增导入批次表、不覆盖既有任务，也不重建或迁移存量数据。
