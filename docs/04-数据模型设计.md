...
activity_type（内部交叉培训/外部技术交流/新技术研究）、topic、activity_date、host_id FK（主讲/组织人）、participant_ids JSONB（org_member id 数组）、output_link、remarks、created_by FK→auth_user。登记即触发主讲/参与培训积分事件；`created_by` 由当前登录账号写入，存量记录从最早创建审计日志幂等回填。删除活动和重算积分均采用软删除，保留来源、审计和历史积分流水。
