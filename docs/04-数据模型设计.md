...
`bug_code`[计]、`title`、`description`、`priority`、`status`、`ci_id` FK、`product_manager_id` FK（所属系统产品经理快照）、`dev_leader_id` FK、`reporter_id`、`source_type/source_id`、复现条件、期望/实际结果、环境、证据、解决说明、验证说明、驳回理由、重开/关闭时间。Bug 使用独立流程，不复用 `problem` 问题管理表。
