...
> 依据 [03-PRD.md](03-PRD.md)（v1.1，已同步 M46）。共 **52 张表**：支撑 18（含组织治理、系统集成配置与飞书服务台可靠同步）、ITSM 12、项目 6、需求 2、流程 4、团队 10。
...
### 2.3 ticket 工单（单表多类型，40 列，创建仅询前单字段必填）
...
### 1.10 feishu_config / feishu_helpdesk_handoff / feishu_helpdesk_intake 飞书服务台（M45/M46）

`feishu_config` 在组织同步和 OAuth 配置之外增加 `helpdesk_id`、`helpdesk_token_encrypted`、`helpdesk_enabled`、`helpdesk_event_verification_token_encrypted`、`helpdesk_event_url` 及事件订阅状态/时间/错误字段；服务台 Token 与事件 Verification Token 使用 Fernet 密文保存，读取接口只返回掩码/是否已配置。事件订阅状态用于区分“凭据已保存”和“服务台事件已成功注册”，避免配置页显示假成功。

`feishu_helpdesk_handoff` 保存一次性交接：`ticket_id`、`action(service_request/requirement)`、`source_user_open_id`、`source_agent_open_id`、`helpdesk_id`、`token_hash`、脱敏 `ticket_snapshot`、`status(issued/consumed/expired)`、`expires_at`、消费单据类型/ID，以及用于卡片回调幂等的 `callback_event_id`。原始令牌不落库，默认 10 分钟过期；用户身份通过 `auth_user.external_id`（Feishu open_id）绑定校验。
`feishu_helpdesk_intake` 是按 `helpdesk_id + ticket_id` 唯一的待分流记录，保存申请人/客服 open_id、当前服务台状态、脱敏快照、`classification(pending/service_request/requirement/cancelled)`、ITOM 关联单据，以及稳定入口的发送时间、渠道（`helpdesk_post/helpdesk_text/im_card_fallback`）和飞书消息 ID。`feishu_helpdesk_sync_event` 保存事件唯一 ID、类型、工单 ID、原始事件、处理状态、尝试次数、下次重试时间和错误；`feishu_helpdesk_outbox` 保存只允许用户看到的 `routing_prompt`、兼容的 `choice_card` 和进展消息、去重键、发送状态及飞书消息 ID。三张表均使用软删除，服务台凭据不进入事件或 outbox。
