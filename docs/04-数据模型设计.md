...
> 依据 [03-PRD.md](03-PRD.md)（v1.2）与 [10-Aily-MCP版本交接与决策上下文.md](10-Aily-MCP版本交接与决策上下文.md)。本文按核心契约分域维护，表数量以 SQLAlchemy 模型和增量迁移代码为准。
> 对照 SN-AOM 106 表；实时指标不使用人工维护的统计预计算表。流程、绩效、配置和用户明确生成的正式报告版本快照仅用于历史可审计与可复现。

### 本次增量：流程运行事实与个人清单偏好

- 不新增业务表或删除历史字段。`auth_user.preferences` JSONB 的可选 `table_views` 对象按清单稳定键保存 `visible` 字段数组、`widths` 列宽映射和 `manual_widths` 手工覆盖字段数组；服务端限制键、数量、列宽范围并要求手工字段存在对应宽度，前端始终保留编号/标题/操作等保护列。未列入 `manual_widths` 的宽度由当前已加载内容和表头在运行时派生，不持久化为业务事实。
- 流程运行时以 `process_task(status=待处理, is_deleted=false)` 的最新任务为当前节点事实。`process_instance.current_step_seq` 及 `Requirement.status`、阶段时间字段是兼容投影和筛选读模型；阶段同步只向前推进，不修改关闭、取消、搁置终态，也不回写历史任务/评分。
- `Requirement.department` 保留为历史兼容字段；新登记接口与新 Excel 模板不再要求或导出该字段。导入服务按表头识别新登记模板与旧版评分模板，旧版列仍可读，评分/决议/人天不会被新登记模板误写。
- 个人待办不落独立快照表。`GET /api/auth/me/todos` 每次从活动流程任务、任务授权和领域实体读取，返回任务编号、单据编号、节点和受控详情链接，避免待办与真实流程状态分叉。
> 本文按核心契约分组说明，不逐项重列全部辅助/兼容表。Aily/MCP 支撑、动态表单、派单和评价明细模型均已实现。数据库事实以真实模型和迁移代码为准。

> M93 服务请求实施交付派单只对既有 `service_dispatch_rule` 和 `ticket` 增加可空/有默认值的字段，不新增或删除业务表，也不回填、迁移或重写任何存量记录；`business_initial_password` 是部署配置，不是持久化业务字段。

> 统一表格内核和本地宽表 UAT 数据集不改变数据库模型：不新增表、字段、索引、约束或迁移。`preferences.table_views.widths` 与 `manual_widths` 只保存用户明确拖拽过的列宽及其字段标记；其他列按当前已加载行和表头的真实渲染宽度在前端派生。前两业务列冻结、短表自然宽度、宽表总宽和 sticky/表体同步均不写回业务字段。`app.scripts.seed_table_uat` 仅在显式 `--confirm-local` 后，使用既有实体写入带专用 `UAT-TABLE` 编号及 `【本地表格UAT】` 标记的纯虚构记录并显式提交；重复执行按业务编号幂等刷新，`cleanup` 只软删除并提交同一专用集合。该工具不属于启动种子、生产初始化或 IDC 数据迁移。

> 需求总览 Excel 导出同样不改变数据模型：只读取现有 `requirement`、关联人员/业务域及派生评分和任务进度，不新增导出快照、字段、索引或迁移，也不持久化筛选条件或扩大记录可见范围。
...
- **business_domain**：code UNIQUE、name、description、owner_id FK org_member（IT 侧 BM，字段非角色）、business_bdo_id FK org_member（业务侧 BDO）、sort、active。`business_bdo_id` 为 M95 新增可空列；只允许绑定已启用、持有 `bdo` 角色且属于该业务域服务部门范围的业务人员。历史 `backup_owner_id` 列保留但不迁移、不读写，避免把旧 IT 备份负责人误识别为 BDO 并确保既有数据不丢失。
...
entity_type、entity_id、action（`VARCHAR(32)`）、actor、summary JSONB（变更字段前后值）、created_at。写审计的服务动作编码必须在入库前由常量和测试保证不超过 32 字符；Aily 通知身份的 OAuth 自动映射与组织补齐分别使用 `auto_map_aily_identity`、`auto_map_aily_identity_org`，避免审计失败回滚已验真的飞书登录事务。

### 1.7 notification_outbox 通知发件箱

event_type、entity_type、entity_id、payload JSONB、channel（in_app/feishu_aily…）、status（pending/sending/sent/failed）、recipient_type、recipient_id、idempotency_key UNIQUE、attempt_count、next_attempt_at、provider_message_id、last_error_redacted、sent_at。站内通知和 Aily 机器人消息共用可靠发件箱；统一通知出口按 ITOM 账号在同一业务事务内写入 `feishu_aily` 文本记录，按事件、实体和账号生成稳定幂等键。账号尚未有活动、已验证的 Aily 机器人身份时，payload 增加内部 `auth_user_id`，收件字段保持为空，状态保留 `pending`，`last_error_redacted=AILY_IDENTITY_NOT_MAPPED`；后台工作器发送前重新解析映射，映射补齐后再填充收件人并投递，等待身份期间不消耗重试次数。机器人未启用时不新增待发记录，消息开关或凭据未就绪时保留 `pending`。`feishu_aily` 的 payload 可为兼容旧记录的 `{text}`，或 `{message_type: interactive, card, fallback_text}`；用户级新记录还可带内部 `auth_user_id`。卡片回调只含公开工单号、动作、评分和幂等键，不保存密钥、JWT、内部主键或敏感业务载荷。本次不新增字段或迁移。
...
entity_type、entity_id、filename、storage_path、size、uploaded_by。合同附件、项目文档、章程原件、Bug 证据、服务请求和 IT 需求补充附件共用；Bug 固定使用 `entity_type=bug`、`entity_id=bug.id`，服务请求正式文件固定使用 `entity_type=ticket`、`entity_id=ticket.id`，需求正式文件固定使用 `entity_type=requirement`、`entity_id=requirement.id`，均不新增专用附件表。建单前临时文件分别使用 `entity_type=ticket_draft` 或 `requirement_draft`、`entity_id=auth_user.id`，只允许上传人随后在同一创建事务绑定或取消；未绑定草稿 24 小时后软删并清理物理文件，不能通过通用清单或下载读取。需求的“其他补充信息”写入既有 `requirement.remarks`，附件元数据不重复写入该列；证据说明文本仍保留在 `bug.evidence`。
...
| 补充附件 [M108] | attachment（通用表，`entity_type=ticket`） | 网页建单使用当前账号临时附件 ID；领域服务在工单创建、流程启动和审计的同一事务校验归属并绑定，附件元数据不写入 `ticket` 列 |
| 受理派单/受理事实 | dispatch_rule_id, dispatch_source, assigned_at, accepted_at | P1 写首节点受理命中规则、来源和派单时间；P2 在首次进入处理中时写实际受理时间 |
| 实施交付派单事实 [M93] | implementation_assignee、implementation_rule_id、implementation_source、implementation_selected_by、implementation_selected_at | 仅服务请求首个受理节点完成时写入；区分受理人本人、指定同事、服务项/目录/全局自动规则、人工队列与流程默认角色，绝不通过上游更正回写 |
...
name、scope_type（service_item/catalog/global）、scope_id、**dispatch_stage（acceptance/implementation）**、target_type（group/member）、target_id、strategy（round_robin/fixed/manual_queue）、priority、active、fallback、last_assigned_member_id、last_assigned_at。服务项不重复保存“当前规则 FK”，而是按 `dispatch_stage + scope_type + scope_id` 解析：服务项规则优先，其次目录默认规则和全局兜底。历史规则迁移后统一为 `acceptance`，不改写历史工单。`implementation` 仅在服务请求受理转交时求值；`manual_queue` 刻意生成未指派流程任务，由下一节点角色认领，而不是重新按默认角色取一个人。两阶段的执行结果各自写入工单事实。
...
| 创建可选 | portfolio_id FK, description, budget_10k, service_item_id FK；project_source、plan_year、decision_level、decision_status、budget_status、external_authorization_amount_cny、decision_reference |
...
project_id FK、parent_task_id FK 自引用、wbs_code[计]（树位置生成）、task_name、assignee FK、start_date、end_date、actual_start、actual_end、status、description、deliverable、predecessors JSONB（任务 id 数组）、progress_pct（0-100% 整数，项目汇总按工期加权）、sort、completed_at（首次达到 100% 的审计时间，完成度后续纠正时不清空）。`actual_end` 是可清空的真实结束日期，不得晚于提交当天或早于 `actual_start`；写入有效 `actual_end` 时领域服务把当前完成度同步为 100%。`completed_locked` 与 `structure_locked` 为接口派生标记，不新增持久化字段：当前 `progress_pct >= 100` 时锁定实际日期编辑、删除、增加子任务和结构变更，不得用历史 `completed_at` 永久锁定任务。经授权用独立请求把完成度纠正到 100% 以下时，任务重新打开并清空 `actual_end`，但保留 `actual_start` 与 `completed_at`；派生能力立即恢复。单行或批量结构移动都只更新既有 `parent_task_id`、同级 `sort` 与派生 `wbs_code`：父子重复选择归一为最高层移动根，后代随根保持内部结构；服务端先锁定项目全部有效 WBS 行，再校验权限、项目范围、引用、当前已完成同级/后代、目标层级和循环关系，任一移动根无效则整个事务回滚。每个实际移动根写一条审计记录，批次锚点和批次数量仅存于审计摘要。本规则不新增业务字段或数据库迁移。
...
project_id FK、wbs_task_id FK 可空、entry_date、`amount_cny NUMERIC(18,2)`（人民币元权威金额）、`amount_10k`（历史兼容投影）、category（software/hardware/service/labor/other/legacy）、cost_type（incurred/committed）、supplier、note、created_by[计]。升级时仅把 `amount_10k × 10000` 精确回填到缺失的 `amount_cny`，不覆盖已有精确金额。

### 3.7 project_budget_item 项目预算分项

project_id FK、category（software/hardware/service/labor/other）、name、`amount_cny NUMERIC(18,2)`、note、created_by[计]。存在分项时预算总额为有效分项之和；没有分项时兼容读取 `project.budget_10k`。

### 3.8 project_effort_entry 项目人天投入

project_id FK、wbs_task_id FK 可空、person_id FK、work_date、`effort_days NUMERIC(8,2)`、role_type（design/development/testing/implementation/pm/operations/other）、`standard_rate_cny_per_day NUMERIC(12,2)` 可空、note、created_by[计]。费率在登记时取请求值或组织配置的角色标准日费率并形成快照；它只用于管理成本，不保存或推导个人薪酬。升级不根据历史任务、工时或人员信息伪造投入事实。

### 3.9 B-OPS 统一投入台账（4 表）

`investment_budget_item`：`lifecycle_stage`（demand/build/run）、`investment_intent`（run/grow/transform）、`subject_type/subject_id`、`period_start/period_end`、category（software/hardware/cloud/network/security/service/outsourcing/telecom/facility/labor/other/legacy）、`cost_nature`（capex/opex）、name、`amount_cny NUMERIC(18,2)`、note、`source_type/source_id`、created_by。`(source_type, source_id)` 唯一，保证历史回填幂等。旧 `project.budget_10k` 迁移为 `legacy_project_budget_total` 兜底行；同一项目存在显式预算分项时不参与汇总。

`investment_cost_entry`：除生命周期、投入目的和归属对象外，保存 `recognition_date`、`amount_cny NUMERIC(18,2)`、`cost_status`（committed/incurred/paid）、category、`cost_nature`、`labor_nature`（none/internal/external/unclassified）、`recurrence`（one_time/recurring）、activity_type、项目/需求/服务项/CI/工单/问题/合同/供应商/WBS 可空引用、供应商快照、note、来源唯一键和 created_by。paid 是 incurred 的支付子集；汇总 incurred 时包含 paid，另列 paid，但二者不相加。合同 `amount_10k` 不回填本表。

`investment_worklog`：保存生命周期、投入目的、归属对象、person_id、work_date、`effort_days NUMERIC(8,2)`、role_type、activity_type、`standard_rate_cny_per_day NUMERIC(12,2)` 快照、项目/需求/服务项/CI/工单/问题/WBS 可空引用、note、来源唯一键和 created_by。禁止未来实际日期，同一人员同一天所有有效行合计不超过 2 人天。工单 `actual_resolution_hours` 与任务计划/估算工时不得自动回填本表。

`investment_allocation`：`source_kind`（budget/cost/worklog）、source_id、target_type/target_id、`percentage NUMERIC(5,2)`、allocation_method、note、created_by；同一来源与目标唯一，同一来源所有有效比例合计不得超过 100%。分摊只在读取目标汇总时施加权重，不复制预算、费用或工时事实。

统一台账归属类型为 project/requirement/service_item/ci/ticket/problem/contract/business_domain/work_task/shared_operations。项目、需求、工单和共享运维成本池的新写入只进入统一表；原 `project_budget_item/cost_entry/project_effort_entry` 保留一个回滚周期并停止新写。历史三表按唯一来源键幂等复制，不删除旧数据。财务实际投入=`incurred`（含 paid）；标准人力估值=人天×登记时费率；管理总投入=财务实际投入+标准人力估值，但存在 `category=labor AND labor_nature=unclassified` 的已发生费用时返回不可用，避免重复计量。

### 3.10 dmc_decision_record DMC/授权决议记录

跨需求和项目的线下治理记录，保存 `entity_type/entity_id`、`decision_level`（`digital_leader`/`eason`/`dmc`）、`decision`、金额、预算来源、条件、决议日期、会议引用、跟进责任人、期限/复核日期、记录人、证据引用及备注。该表只记录已在线下完成的授权或 DMC 决议，不实现在线投票；写入时仍按关联需求/项目权限复核并产生审计。
...
`Requirement.score_d1_strategy`、`score_d2_value`、`score_d3_tech`、`score_d4_org`、`score_d5_risk` 是当前五维共识分；`score_d4_org` 作为兼容列名，但业务含义为“业务成熟度”（组织与流程成熟度合并）。`score_d6_speed` 与 `RequirementScore.d6_speed` 仅保留历史六维记录。`RequirementScoringConfig.weights` 保存当前五维权重，`legacy_weights` 保存历史六维权重快照；读取服务依据 D6 是否存在选择计算口径，不回写历史分值或显示版本标签。
...
| 评估阶段 | score_d1_strategy … score_d5_risk（当前五维）；score_d6_speed 仅历史兼容；decision、solution_type、prd_effort、dev_effort |
| 分析阶段 | moscow、owner FK、target_date、solution、acceptance_criteria JSONB |
| 实现阶段 | project_id FK（可选挂接）、implementation_route（可空路径快照：需求开发实现/转项目管理；执行分流时写入，不回填历史） |
| 派生 [计] | requirement_code、status（registered/**supplementing**/evaluating/analyzing/implementing/closed 等）、requester/requester_name、registered_at/evaluating_at/analyzing_at/implementing_at/closed_at、closure_note；`supplementing` 表示审批明确退回原登记人补充，不是关闭或取消 |

### 4.2 requirement_task 需求任务

`task_code`（唯一、非空，`RT-YYYYMM-NNNN`）、`requirement_id` FK（可空，允许先登记后关联）、`registrar` FK（可空，兼容管理员账号无人员绑定）、`name`、`description`、`assignee` FK、`plan_date`、`plan_effort`、`actual_effort`、`status`（待处理/进行中/已完成）、`done_at`[计]，以及基类持久化的 `created_at` 登记时间。页面直接登记时 `registrar` 取当前登录账号关联人员；从某条需求创建/转化时取该需求原登记账号关联人员，不得误记执行转化的 IT 人员。同一需求允许多行任务，独立任务后续可编辑补充关联；任务继续使用 `GlidBase.is_deleted` 软删除。启动期迁移把 PostgreSQL 的 `requirement_id` 约束改为可空，并只为 `task_code` 缺失的历史行按原 `created_at` 月份及 `created_at + id` 稳定顺序补齐 RT 编号；不覆盖、删除或重建存量行，模板导入也不新增批次表、不覆盖既有任务。

### 4.3 bug 缺陷单

`bug_code`[计]、`title`、`description`、`priority`、`status`、`ci_id` FK（所属系统选项来自 CMDB）、`product_manager_id` FK（所属系统产品经理快照）、`dev_leader_id` FK、`reporter_id`（也是 Bug 修复清单的登记人）、`source_type/source_id`、复现条件、期望/实际结果、环境、`evidence` 说明文本、解决说明、验证说明、驳回理由、重开/关闭时间。截图、日志等二进制证据通过通用 `attachment(entity_type=bug, entity_id=bug.id)` 关联，不重复持久化文件元数据。Bug 使用独立流程，不复用 `problem` 问题管理表。

### 4.4 bug_fix_task Bug 修复任务

`task_code`（唯一、非空，`BT-YYYYMM-NNNN`）、`bug_id` FK、`name`、`task_type`（开发/测试/其他）、`description`、`assignee` FK、`plan_start`、`plan_date`、`plan_effort`、`actual_effort`、`status`（登记/排期/执行/暂停/关闭）、`done_at`、`completion_note`，以及基类持久化的 `created_at` 登记时间。一个 Bug 允许多条开发或测试任务，全部必要子任务关闭后才允许进入产品经理验证；父级 Bug 继续使用 `BG-YYYYMM-NNNN`，不与子任务编号混用。启动迁移只为缺失编号的历史子任务按原登记月份及稳定创建顺序补齐 BT 编号，不改变父子关系或任何业务字段。
...
### 4.6 project_development_task 项目开发任务

`task_code`（唯一、非空，`PT-YYYYMM-NNNN`）、`project_id` FK（必填）、`wbs_task_id` FK（可空，后续可补充）、`title`、`description`、`acceptance_criteria`、`task_type`、`registrar` FK、`assignee` FK、`priority`、`environment`、`version`、计划起止日期、计划/实际人天、`status`（待处理/进行中/已完成，唯一状态权威）、`completion_note`、`done_at`，以及基类持久化的 `created_at` 登记时间。页面直接登记时 `registrar` 取当前登录人员；由项目或 WBS 转化创建时取项目经理。项目和 WBS 的所属关系由服务端校验；非管理员只能删除待处理任务，管理员可按审计规则删除其他状态任务。完成度不是本表的第二个状态字段，而是读取时根据 `task_progress_entry` 派生：已完成固定为 100%，其他状态取最新非空百分比且最多为 99%。

### 4.7 task_progress_entry 任务追加式进度

`task_kind`、`task_id`、`author_id` FK（可空）、`author_name`（写入时姓名/账号快照）、`progress_percent`（可空，0–100）、`status_snapshot`、`comment`、`created_at`。当前用于委派任务和项目开发任务。记录只追加、不提供更新/覆盖接口；列表按创建时间及时间可排序 GLID 双重倒序，保证同秒写入仍稳定展示最新记录。通知幂等键使用进度记录 ID，连续多次更新不会互相覆盖。项目开发的普通进度记录只允许 0–99%；100% 仅由显式完成动作生成，并与主任务的已完成状态、完成说明及完成时间同事务提交。该规则不要求迁移或改写历史记录。
...
definition_id FK、seq、step_code（版本内稳定节点编码）、name、node_type（processing 处理节点 / approval 审批节点）、default_role（R 主责：it_bp/it_pdm/it_pm/it_dev/it_ops/is_mgr/manager；Bug 开发修复节点为空，由修复子任务负责人实际执行）、cc_roles（I 知会角色/用户组）、autonomy_level（L1-L4）、sla_hours、description。已有实例后不可就地改变 step_code、节点顺序/类型、处理人、自治级别或 SLA；`cc_roles` 是唯一允许原地维护的非阻塞通知规则，仅对保存后首次激活的节点生效。已生成任务的 `raci_snapshot.informed` 和既有通知不回写。

### 5.3 process_instance 流程实例

definition_id FK、entity_type、entity_id（触发单据）、status（进行中/已完成/已终止；需求专项还使用 `returned` 表示等待登记人补充）、current_step_seq[计]、started_at、completed_at。需求退回某个已到达流程节点时实例保持 `running` 并重新生成目标节点待办；只有“登记人补充”使用 `returned` 且不生成伪流程任务。重新提交在同一实例恢复 `running`，不创建第二个需求或第二个流程实例。

### 5.4 process_task 流程任务

instance_id FK、step_id FK、definition_version、step_code_snapshot、raci_snapshot JSONB（任务生成时的 R/A/C/I 快照）、assignee FK（默认角色解析，可转派）、status（待处理/进行中/已完成/已驳回/已跳过）、started_at、due_at[计]（按步骤 SLA）、`viewed_at`、`viewed_by` FK（当前处理人的首次实际查阅事实）、`upstream_correction_enabled`（新任务为 true；升级前存量待办默认为 false）、completed_at、completed_by FK、comment。转派复用 `assignee`，不新增业务字段：当前待办处理人或管理员只能选择在岗且具有有效系统账号的人员；流程节点、状态和 `raci_snapshot` 保持不变，原/新处理人及可选理由写入审计，通知新处理人。`started_at` 仅代表任务生成/派发，不能替代实际查阅；快照和查阅事实共同保证流程版本、绩效口径及回改窗口的历史可审计性。需求审批驳回后原任务保留 `已驳回`、实际处理人和带目标节点的意见；若退回已到达前序节点，则新增一条该步骤的待办而不软删旧任务。候选节点由同一实例非删除任务的历史 `step_id` 推导，不另建目标快照表。旧版误终止的需求只修复同一需求的最新有效 `status=rejected` 实例，不处理已关闭、已取消或已搁置需求，不复活旧流程实例，也不覆盖历史任务、评分、附件或审计。
...
实时指标只存在于 `services/reporting.py` 的注册表和查询结果，不持久化为月/季统计行。以下表只承载用户明确创建的模板、正式报告、不可变版本、受众、幂等生成事实和未来调度配置。

### 6A.1 report_template

code（唯一）、name、description、domains JSONB、metric_codes JSONB、default_filters JSONB、default_period_type（week/month/quarter/half_year/year/custom）、is_system、active、created_by。模板只能引用服务端注册且当前操作者有权使用的指标。

### 6A.2 report_instance

template_id FK、title、period_type、period_start、period_end、filters JSONB、status（draft/review/approved/published/locked）、current_version、published_version、created_by FK、process_instance_id FK、published_at、locked_at。周期和筛选属于报告实例合同；发布只指向一个已生成版本，不覆盖历史版本。

### 6A.3 report_version

report_instance_id FK + version 唯一、status、metric_snapshot JSONB、narrative JSONB、formula_versions JSONB、data_quality JSONB、checksum、generated_by、generated_at、locked_at。快照仅在显式生成正式报告版本时创建；发布后锁定，后续修订递增 version，不修改旧行。

### 6A.4 report_audience

report_instance_id FK、subject_type（user/role/group）、subject_id，三元组唯一。它只约束已发布报告受众，不能授予源模块或敏感指标权限。

### 6A.5 report_generation_job

report_instance_id FK、actor_id FK、idempotency_key、request_digest、status、version_id FK、error_code、completed_at；`actor_id + idempotency_key` 唯一。同键同摘要恢复首次版本，同键异摘要冲突。

### 6A.6 report_schedule

template_id FK、name、period_type、enabled、next_run_at、audience JSONB、created_by。B2 首期只预留模型，默认禁用且不提供执行 API/调度器，不能据此宣称自动周报/月报已启用。

### 6A.7 组织报表口径配置

`org_settings` 增加 `reporting_timezone`（默认 Asia/Shanghai）、`reporting_week_start`（1=周一）、`fiscal_year_start_month`（默认 1）、`workday_hours`（默认 8）、`report_min_group_size`（默认 3）和 `report_role_rates` JSONB（角色标准日费率）。费率是管理口径而非个人薪酬；分组阈值预留给后续人员聚合，当前人员明细仍由 `reports_people` 显式保护。

## 6B. 平台产品运营中枢（P0 已实现；P1/P2 为目标模型）

P0 已新增以下前四张增量表；`service_item`、`requirement`、统一投入和正式报告继续作为各自事实的唯一内部来源，不自动回填或推断平台语义。P1/P2 所列赋能、外部引用和观测表仍是目标模型，当前数据库不存在。

### 6B.1 platform_service_profile

`service_item_id` FK/唯一、`owner_id` FK、`lifecycle`（candidate/pilot/active/retiring/retired）、`value_proposition`、`management_scope` JSONB、`enabled_at`、`created_by/updated_by`、时间戳、软删除。它是现有服务项的可选 1:1 平台产品档案，不复制名称、目录、SLA 或服务表单。

### 6B.2 platform_demand_profile

`requirement_id` FK/唯一、`service_item_id` FK、`business_domain_id` FK、`demand_class`、`expected_outcome`、`target_quarter`、`capacity_class`、审计字段、软删除。需求状态、负责人和流程仍来自 `requirement`，档案只增加平台统筹语义。

### 6B.3 platform_capacity_plan / platform_capacity_commitment

计划表已保存服务、季度、版本、状态（draft/review/approved/superseded）、总人天、计划不可用、BAU 预留、风险缓冲、净人天、审批人/理由/时间、上一版本及 `created_by + idempotency_key + request_digest`；`service_item_id + period + version` 唯一。承诺表已保存 `plan_id`、归属对象类型/ID、承诺类型（demand/roadmap/reliability/enablement）、承诺人天、`lifecycle_stage`、`investment_intent`、负责人、状态、超容量审批事实和操作者级幂等字段。已批准/已替代版本不可原位修改；修订新增递增版本并复制原承诺，保留旧行。承诺合计默认不得超过净容量，超容量仅 CIO 可记录例外理由和审批事实。

### 6B.4 platform_enablement_asset / platform_service_objective（P1/P2 目标，未实现）

赋能资产保存类型、名称、说明、关联服务、负责人、版本、状态、目标受众、内部附件或外部引用及审计字段。服务目标保存服务、周期、指标键、目标值、单位、公式版本、状态和审计字段；一个服务/周期/指标键/公式版本唯一。

### 6B.5 external_system / external_work_reference（P1/P2 目标，未实现）

外部系统保存稳定来源代码、名称、类型、负责人、同步模式、预期新鲜度、启停状态和非敏感连接元数据；凭据必须使用现有受控密钥机制，不写明文、不返回密钥。外部引用保存内部对象类型/ID、外部系统、外部记录类型、稳定外部 ID、受控 URL、状态摘要、来源更新时间及审计字段；同一内部对象、来源和外部 ID 唯一。它不替代 ITOM 内部记录关联，也不把外部记录声明为 ITOM 主记录。

### 6B.6 external_sync_run / platform_metric_observation（P1/P2 目标，未实现）

同步运行保存来源、开始/结束时间、水位、读取/成功/失败数量、结果、错误代码/摘要和执行者。观测保存指标键、服务、周期、数值、单位、来源时间、接收时间、公式版本、质量（ok/stale/error/conflict/no_data）、外部引用/同步运行和来源证据摘要。观测采用不可变追加或受控更正版本；正式报告保存所用观测 ID，后续更正不改写已发布版本。

### 6B.7 关系与删除约束

- 服务项/需求删除或停用时，平台档案与历史计划、承诺、观测不得物理级联删除；
- 退役平台服务保留需求、投入、承诺、目标和报告引用；
- 没有有效分母的观测保存 `value=null, quality=no_data`，不得保存伪造的 `0%`；
- 外部同步错误、冲突和过期状态必须保留，不得用最近一次正常值覆盖来源事实；
- P0 四表由现有启动建表机制增量创建，不回填存量服务项或需求、不修改旧表；回滚旧应用镜像时保留这些闲置表和数据，不执行破坏性降级。P1/P2 字段、索引、迁移与回滚仍须在各自实现阶段以[专项设计](superpowers/specs/2026-08-31-platform-product-operations-hub-design.md)和 [ADR-0001](adr/0001-platform-product-operations-overlay.md)为基线补齐。
...
erDiagram
    ...
    project ||--o{ project_budget_item : "预算分项"
    project ||--o{ project_effort_entry : "人天投入"
    ...
    report_template ||--o{ report_instance : "实例化"
    report_instance ||--o{ report_version : "不可变版本"
    report_instance ||--o{ report_audience : "发布受众"
    report_instance ||--o{ report_generation_job : "幂等生成"
```
...
| 原则 | 落实 |
| --- | --- |
| 实时指标无预计算表 | 业务分析不使用手工统计表；project 计算列与 performance_score 为系统维护结果。`report_version` 只保存用户明确生成的正式报告审计快照，不参与实时指标回写或作为领域事实来源 |
...
`org_settings` 为单行配置：`digital_team_department_ids` 保存数字化团队根部门数组，`digital_team_member_ids` 保存从其他/混合组织中单独纳入的人员数组，`digital_team_include_children` 控制是否展开所选部门后代。有效数字化团队为“部门展开后的在岗成员 ∪ 单独指定的在岗人员”；指定人员不会把其所在部门的其他人员一并纳入。`feishu_auto_sync_enabled`、`feishu_auto_sync_interval_minutes` 与 `feishu_auto_sync_last_attempt_at` 控制飞书自动同步；`reporting_timezone/reporting_week_start/fiscal_year_start_month/workday_hours/report_min_group_size/report_role_rates` 保存统一报表口径与角色标准日费率。配置独立于飞书凭据表，避免更换组织来源时丢失内部团队和报表口径。
...
### 1.16 软件发布清单（B1，非数据库模型）

- `release/current.json` 只保存 schema 版本和当前不可变清单文件名；文件名必须是安全 basename，禁止路径穿越。
- `release/releases/v<SemVer>.json` 保存产品双语名称/版本、发布通道/状态/日期、回滚兼容声明及中英文更新摘要；清单纳入 Git 审计，已发布版本不覆盖修改。
- 运行时使用 Pydantic 严格校验，CI 同时校验 SemVer、文件名、当前指针、前端包版本和双语更新日志。
- `ui_branding_version.config.legal` 增加开发者、厂商、版权、官网、支持、许可及第三方声明字段，仍存于既有 JSON 配置，不新增列或迁移。品牌配置版本与软件版本相互独立。
- B1 不增加发布、部署或阅读回执表，不改变 `auth_user`、组织、RBAC、Aily/MCP 或业务单据模型。
