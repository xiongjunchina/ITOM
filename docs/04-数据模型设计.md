...
- 不新增业务表或删除历史字段。`auth_user.preferences` JSONB 增加可选 `table_views` 对象，按清单稳定键保存 `visible` 字段数组和 `widths` 列宽映射；服务端限制键、数量和列宽范围，前端始终保留编号/标题/操作等保护列。
...
`portfolio_code`[计]、name、owner_id FK、year、description、status（draft/active/archived）、planning_start、planning_end、budget_limit_10k、sort。

新建固定为 draft，成功发布组合基线后转 active；非空组合禁止归档。既有组合启动时按稳定顺序幂等补 `PF-LEGACY-NNNN`，新组合使用 `PF-*` 业务编码。

### 3.1.1 portfolio_objective 组合目标

portfolio_id FK、objective_code、name、description、metric_name、target_value、current_value、weight（0–100）、owner_id FK、period_start/end、status。活动目标权重合计由服务层限制为不超过 100%；同一组合活动目标编码不得重复。

### 3.1.2 portfolio_project 组合成员治理

portfolio_id FK、project_id FK、governance_status、system_score[计]、priority_rank、proposal_reason、decision_reason、objective_contributions JSONB、decided_by、decided_at。`objective_contributions` 结构为 `[{objective_id, weight, note}]`，项目贡献权重合计不超过 100%。

首期一个项目只有一个活动主要组合。PostgreSQL 使用 `WHERE is_deleted=false` 的部分唯一索引约束活动 `project_id`、活动 `(portfolio_id, project_id)` 和组合内活动 `priority_rank`（空排序除外）；不能使用普通 UNIQUE，否则软删除成员会阻止项目迁移后再次加入。`project.portfolio_id` 保留为兼容投影，成员迁移保留旧软删除行和治理动作。

### 3.1.3 portfolio_scoring_rule / portfolio_project_score

- `portfolio_scoring_rule`：portfolio_id、dimension_code、name、description、weight（0–100）、evidence_required、active、sort；同一组合维度编码唯一。
- `portfolio_project_score`：portfolio_project_id、rule_id、score（0–100）、evidence、scored_by、scored_at；每个活动成员/维度保留一行最新分数。

系统分仅在所有启用维度已评分且启用权重恰为 100% 时计算；治理动作和已发布基线保存历史证据。

### 3.1.4 project_dependency 跨项目依赖

predecessor_project_id、successor_project_id、dependency_type、deliverable、due_date、owner_id、impact、status、description。数据库禁止自依赖，领域服务禁止活动依赖环；WBS 内部依赖仍在 `wbs_task.predecessor_ids`。

### 3.1.5 project_resource_commitment 资源承诺

project_id、person_id、role_name、start_date、end_date、allocation_percent（1–100）、planned_person_days、note。结束日期不得早于开始日期；组合看板按同一人员重叠区间聚合，超过 100% 只产生冲突告警，不落自动排程结果。

### 3.1.6 portfolio_governance_action 治理动作

portfolio_id、portfolio_project_id（可空）、action、reason、before_value JSONB、after_value JSONB、actor_id、effective_at。该表追加记录迁入/迁出、目标贡献、评分、状态决策和基线发布，不随项目归档删除。

### 3.1.7 portfolio_baseline 不可变基线

portfolio_id、version、snapshot JSONB、published_by、published_at；`(portfolio_id, version)` 唯一。每次发布新增递增版本，快照经 JSON 编码后保存组合、目标、评分、排序、项目派生指标、依赖和资源承诺，旧版本不更新。
...
> `progress_pct` / `health_status` / `actual_cost_10k` 是 API 实时派生值，不在 `project` 表持久化；组合看板复用同一 `compute_metrics` 口径，不建立第二套项目统计事实。
...
erDiagram
    ...
    vendor ||--o{ contract : ""
    vendor ||--o{ ci : ""
    portfolio ||--o{ project : "主要组合兼容字段"
    portfolio ||--o{ portfolio_objective : "战略目标"
    portfolio ||--o{ portfolio_project : "治理成员"
    portfolio ||--o{ portfolio_scoring_rule : "评分规则"
    portfolio ||--o{ portfolio_baseline : "不可变基线"
    portfolio ||--o{ portfolio_governance_action : "治理动作"
    project ||--o{ portfolio_project : "活动成员唯一"
    portfolio_project ||--o{ portfolio_project_score : "维度评分"
    project ||--o{ project_dependency : "前置或后续"
    project ||--o{ project_resource_commitment : "资源承诺"
    project ||--o{ wbs_task : ""
    project ||--o{ milestone : ""
    project ||--o{ risk : ""
