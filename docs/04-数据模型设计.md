...
> 依据 [03-PRD.md](03-PRD.md)（v1.1，已同步 M44）。共 **51 张表**：支撑 17（含组织治理与系统集成配置）、ITSM 12、项目 6、需求 2、流程 4、团队 10。
...
`org_settings` 为单行配置：`digital_team_department_ids` 保存数字化团队根部门数组，`digital_team_include_children` 控制是否展开后代；`feishu_auto_sync_enabled`、`feishu_auto_sync_interval_minutes` 与 `feishu_auto_sync_last_attempt_at` 控制飞书自动同步。独立于飞书凭据表，避免更换组织来源时丢失内部团队口径。
