...
`requirement_id` FK（可空，允许先登记后关联）、`registrar` FK（可空，兼容管理员账号无人员绑定）、`name`、`description`、`assignee` FK、`plan_date`、`plan_effort`、`actual_effort`、`status`（待处理/进行中/已完成）、`done_at`[计]。同一需求允许多行任务，独立任务后续可编辑补充关联；任务继续使用 `GlidBase.is_deleted` 软删除。启动期迁移仅把 PostgreSQL 的 `requirement_id` 约束改为可空，不回填、删除或重建存量行；模板导入不新增批次表、不覆盖既有任务。

### 4.3 bug 缺陷单

`bug_code`[计]、`title`、`description`、`priority`、`status`、`ci_id` FK（所属系统选项来自 CMDB）、`product_manager_id` FK（所属系统产品经理快照）、`dev_leader_id` FK、`reporter_id`、`source_type/source_id`、复现条件、期望/实际结果、环境、`evidence` 说明文本、解决说明、验证说明、驳回理由、重开/关闭时间。截图、日志等二进制证据通过通用 `attachment(entity_type=bug, entity_id=bug.id)` 关联，不重复持久化文件元数据。Bug 使用独立流程，不复用 `problem` 问题管理表。
...
`task_code`[计]、`project_id` FK（必填）、`wbs_task_id` FK（可空，后续可补充）、`title`、`description`、`acceptance_criteria`、`task_type`、`registrar` FK、`assignee` FK、`priority`、`environment`、`version`、计划起止日期、计划/实际人天、`status`（待处理/进行中/已完成）、`completion_note`、`done_at`。项目和 WBS 的所属关系由服务端校验；非管理员只能删除待处理任务，管理员可按审计规则删除其他状态任务。
...
instance_id FK、step_id FK、definition_version、step_code_snapshot、raci_snapshot JSONB（任务生成时的 R/A/C/I 快照）、assignee FK（默认角色解析，可改派）、status（待处理/进行中/已完成/已驳回/已跳过）、started_at、due_at[计]（按步骤 SLA）、`viewed_at`、`viewed_by` FK（当前处理人的首次实际查阅事实）、`upstream_correction_enabled`（新任务为 true；升级前存量待办默认为 false）、completed_at、completed_by FK、comment。`started_at` 仅代表任务生成/派发，不能替代实际查阅；快照和查阅事实共同保证流程版本、绩效口径及回改窗口的历史可审计性。需求审批驳回后原任务保留 `已驳回`、实际处理人和带目标节点的意见；若退回已到达前序节点，则新增一条该步骤的待办而不软删旧任务。候选节点由同一实例非删除任务的历史 `step_id` 推导，不另建目标快照表。旧版误终止的需求只修复同一需求的最新有效 `status=rejected` 实例，不处理已关闭、已取消或已搁置需求，不复活旧流程实例，也不覆盖历史任务、评分、附件或审计。
