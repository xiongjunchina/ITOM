...
entity_type、entity_id、filename、storage_path、size、uploaded_by。合同附件、项目文档、章程原件和 Bug 证据共用；Bug 固定使用 `entity_type=bug`、`entity_id=bug.id`，不新增 Bug 专用附件表，证据说明文本仍保留在 `bug.evidence`。
