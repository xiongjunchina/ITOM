...
`requirement_id` FK（可空，允许先登记后关联）、`registrar` FK（可空，兼容管理员账号无人员绑定）、`name`、`description`、`assignee` FK、`plan_date`、`plan_effort`、`actual_effort`、`status`（待处理/进行中/已完成）、`done_at`[计]。页面直接登记时 `registrar` 取当前登录账号关联人员；从某条需求创建/转化时取该需求原登记账号关联人员，不得误记执行转化的 IT 人员。同一需求允许多行任务，独立任务后续可编辑补充关联；任务继续使用 `GlidBase.is_deleted` 软删除。启动期迁移仅把 PostgreSQL 的 `requirement_id` 约束改为可空，不回填、删除或重建存量行；模板导入不新增批次表、不覆盖既有任务。
...
`task_code`[计]、`project_id` FK（必填）、`wbs_task_id` FK（可空，后续可补充）、`title`、`description`、`acceptance_criteria`、`task_type`、`registrar` FK、`assignee` FK、`priority`、`environment`、`version`、计划起止日期、计划/实际人天、`status`（待处理/进行中/已完成，唯一状态权威）、`completion_note`、`done_at`。页面直接登记时 `registrar` 取当前登录人员；由项目或 WBS 转化创建时取项目经理。项目和 WBS 的所属关系由服务端校验；非管理员只能删除待处理任务，管理员可按审计规则删除其他状态任务。完成度不是本表的第二个状态字段，而是读取时根据 `task_progress_entry` 派生：已完成固定为 100%，其他状态取最新非空百分比且最多为 99%。
