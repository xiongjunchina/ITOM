...
- `ai_provider_config`：唯一 `code`、提供方类型、模型连接、加密 API Key、超时/输出限制、能力探测、主备和 `enabled=false`；探测状态可查询，但密钥不进入普通读取结果。
- `ai_agent_profile` / `ai_agent_profile_version`：唯一档案 `code`、受众、当前活动默认模型、最高风险等级、启用状态和 `retention_days`；档案保留期数据库默认 30 天，并由数据库 `CHECK` 约束为 0–90 天。版本以 `(profile_id, version)` 唯一，保存中英文系统指令、已启用能力、知识范围、发布人、发布时刻及 `config_snapshot`。该 JSON 快照只保存发布控制字段 `name/default_provider_id/retention_days/enabled`；草稿 PATCH 只修改版本快照，成功发布/回滚才把快照和版本风险原子应用到活动档案行。
- `ai_conversation` / `ai_message`：网页当前 `auth_user` 的档案/版本、语言、白名单页面上下文、生命周期状态，以及脱敏结构化消息、角色、Token 用量和耗时。
- `ai_action`：会话、发起账号、能力、风险等级、规范化参数摘要、确认凭证哈希、状态、结果和关联实体；`(auth_user_id, capability_code, idempotency_key)` 唯一，避免同一用户对同一能力重复执行。
- `ai_provider_call`：提供方、会话/消息/档案版本、模型、用途、输入/输出 Token、耗时、结果码、状态及脱敏错误元数据。

能力处理器由代码注册，数据库只能关闭已注册能力，不能创建可执行处理器。启动迁移 `ensure_assistant_schema()` 仅在 PostgreSQL 执行 `CREATE TABLE IF NOT EXISTS`、缺列补齐（包括 `ai_agent_profile_version.config_snapshot JSONB NOT NULL DEFAULT '{}'`）、`CREATE INDEX IF NOT EXISTS` 及幂等追加的保留期 `CHECK` 约束；SQLite 测试库仍由 `Base.metadata.create_all()` 建表。迁移不回填、不重算、不改写既有业务数据、流程实例、Aily 外部身份或 MCP 审计；Task 4 在尚未推送/部署的评审基线上补齐快照列，因此不存在需要重解释的已发布生产档案版本。对话默认保留 30 天并允许管理员在 0–90 天内配置；业务动作审计独立按审计策略保留。完整字段与安全约束见 [`docs/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md)。
