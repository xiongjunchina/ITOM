...
event_type、entity_type、entity_id、payload JSONB、channel（in_app/feishu_aily…）、status（pending/sending/sent/failed）、recipient_type、recipient_id、idempotency_key UNIQUE、attempt_count、next_attempt_at、provider_message_id、last_error_redacted、sent_at。站内通知和 Aily 机器人消息共用可靠发件箱；统一通知出口会在同一业务事务内为已映射收件人写入 `feishu_aily` 文本记录，按事件、实体和收件身份生成稳定幂等键；消息开关或凭据未就绪时保留 `pending`，后台工作器负责发送和指数退避。`feishu_aily` 的 payload 可为兼容旧记录的 `{text}`，或 `{message_type: interactive, card, fallback_text}`。卡片回调只含公开工单号、动作、评分和幂等键，不保存密钥、JWT、内部主键或敏感业务载荷。本次不新增字段或迁移。
