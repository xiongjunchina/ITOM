...
> 依据 [03-PRD.md](03-PRD.md)（v1.2）与 [10-Aily-MCP版本交接与决策上下文.md](10-Aily-MCP版本交接与决策上下文.md)。P1 当前代码实际映射 **80 张表**：支撑 29、ITSM 15、项目 6、需求 4、流程 4、团队 22。P0 删除 4 张 Helpdesk 表并新增 4 张 MCP 支撑表；P1 新增服务项表单版本和派单规则 2 张表；P2 评价明细表完成后目标为 **81 张表**。
> 对照 SN-AOM 106 表；不使用人工维护的统计预计算表。流程、绩效和配置版本快照仅用于历史可审计与可复现。
> 本文按核心契约分组说明，不逐项重列全部辅助/兼容表。Aily/MCP 支撑、动态表单和派单模型已实现；评价明细仍是 P2 目标。数据库事实以真实模型和迁移代码为准。
...
## 2. ITSM 域（P1 当前 15 表；P2 目标 16 表）
...
| 派单事实 [P1 模型] | dispatch_rule_id, dispatch_source, assigned_at, accepted_at | P1 写命中规则、来源和派单时间；实际受理时间在 P2 写入 |
| 用户确认 | confirmation_due_at [P2], suspected_major_impact [P1] | 待确认期限和疑似大范围影响标记 |
