...
> 依据 [03-PRD.md](03-PRD.md)（v1.2）与 [10-Aily-MCP版本交接与决策上下文.md](10-Aily-MCP版本交接与决策上下文.md)。P0 当前代码实际映射 **78 张表**：支撑 29、ITSM 13、项目 6、需求 4、流程 4、团队 22。P0 已删除 4 张 Helpdesk 表并新增 4 张 MCP 支撑表；P1 还将新增 3 张 ITSM 表，最终目标为 **81 张表**。
> 对照 SN-AOM 106 表；不使用人工维护的统计预计算表。流程、绩效和配置版本快照仅用于历史可审计与可复现。
> 本文按核心契约分组说明，不逐项重列全部辅助/兼容表。Aily/MCP 支撑模型和发件箱扩展已在 P0 实现；动态表单、派单和评价模型仍是 P1/P2 目标。数据库事实以真实模型和迁移代码为准。
...
event_type、entity_type、entity_id、payload JSONB、channel（in_app/feishu_aily…）、status（pending/sending/sent/failed）、recipient_type、recipient_id、idempotency_key UNIQUE、attempt_count、next_attempt_at、provider_message_id、last_error_redacted、sent_at。站内通知和 Aily 机器人消息共用可靠发件箱；外部失败可重试，但不得保存密钥或完整敏感载荷。
...
单行配置：enabled、mcp_auth_mode、mcp_jwt_secret_encrypted、allowed_tenant_ids JSONB、allowed_agent_ids JSONB、allowed_origins JSONB、bot_app_id、bot_app_secret_encrypted、api_base、message_enabled、last_test_at、last_test_status、last_error_redacted。所有密钥以 Fernet 密文保存，读取接口只返回是否已配置，不返回密文或明文。
...
### 1.13 mcp_operation_intent 写操作确认与幂等 [P0 表结构已实现，P1 启用]

intent_id UNIQUE、tool_name、auth_user_id、normalized_payload JSONB、payload_digest、token_hash、idempotency_key、status（prepared/consumed/expired/failed）、expires_at、consumed_at、result_entity_type、result_entity_id、result_snapshot JSONB。唯一约束 `(auth_user_id, tool_name, idempotency_key)`；原始确认凭证不落库，重复提交返回首次执行结果。

---

## 2. ITSM 域（P0 当前 13 表；P1/P2 目标 16 表）
...
| search_keywords / search_synonyms | JSONB | Aily 检索词和同义表达 [目标] |
| typical_scenarios / exclusion_scenarios | JSONB | 典型适用与排除场景 [目标] |
| active_form_version_id | FK→service_item_form_version | 当前表单版本 [目标] |
| process_definition_id | FK→process_definition | 当前绑定流程 [目标] |
| dispatch_rule_id | FK→service_dispatch_rule | 当前派单规则 [目标] |
| approval_required / default_priority_rule | BOOLEAN / JSONB | 审批和默认优先级规则 [目标] |
...
| 动态表单 [目标] | request_data JSONB, request_form_version_id, request_form_snapshot JSONB | 保存答案和提交时定义快照，服务项改版不改写历史 |
| 派单事实 [目标] | dispatch_rule_id, dispatch_source, assigned_at, accepted_at | 命中规则、来源、派单与实际受理时间 |
| 用户确认 [目标] | confirmation_due_at, suspected_major_impact | 待确认期限和疑似大范围影响标记 |
...
### 2.13 service_item_form_version 服务项表单版本 [配][目标]
...
### 2.14 service_dispatch_rule 服务派单规则 [配][目标]

name、scope_type（service_item/catalog/global）、scope_id、target_type（group/member）、target_id、strategy（round_robin/fixed/manual_queue）、priority、active、fallback、last_assigned_member_id、last_assigned_at。服务项规则优先，其次目录默认规则和全局兜底；规则执行结果写入工单派单快照。

### 2.15 ticket_satisfaction 工单评价 [目标]

ticket_id FK UNIQUE、score（1-5）、tags JSONB、comment、source（web/aily）、rated_by FK→auth_user、rated_at、version、updated_at。每张工单只有一条有效评价；修改必须写审计。`ticket.satisfaction` 继续保存当前 score 以兼容既有统计。
...
erDiagram
    ...
    service_item }o--|| service_dispatch_rule : "派单规则"
