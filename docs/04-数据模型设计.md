...
网页智能体采用新增结构，不复用或改写 `aily_integration_config`：`ai_provider_config` 保存模型连接、加密密钥、能力探测、主备与启用状态；`ai_agent_profile` / `ai_agent_profile_version` 保存受众档案、最高能力等级、版本化中英文指令、能力/知识配置及发布回滚；`ai_conversation` / `ai_message` 保存当前 ITOM 用户的网页会话和脱敏结构化消息；`ai_action` 保存能力、风险、参数摘要、确认状态、结果与关联实体；`ai_provider_call` 只保存模型、Token、耗时、结果码和脱敏错误元数据。

能力处理器由代码注册，数据库只能关闭已注册能力，不能创建可执行处理器。迁移只新增表、索引和默认权限，不回填、不重算、不改写既有业务数据、流程实例、Aily 外部身份或 MCP 审计。对话默认保留 30 天并允许管理员在 0–90 天内配置；业务动作审计独立按审计策略保留。完整字段与安全约束见 [`docs/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md)。
