...
`task_code`[计]、`project_id` FK（必填）、`wbs_task_id` FK（可空，后续可补充）、`title`、`description`、`acceptance_criteria`、`task_type`、`registrar` FK、`assignee` FK、`priority`、`environment`、`version`、计划起止日期、计划/实际人天、`status`（待处理/进行中/已完成）、`completion_note`、`done_at`。页面直接登记时 `registrar` 取当前登录人员；由项目或 WBS 转化创建时取项目经理。项目和 WBS 的所属关系由服务端校验；非管理员只能删除待处理任务，管理员可按审计规则删除其他状态任务。

### 4.7 task_progress_entry 任务追加式进度

`task_kind`、`task_id`、`author_id` FK（可空）、`author_name`（写入时姓名/账号快照）、`progress_percent`（可空，0–100）、`status_snapshot`、`comment`、`created_at`。当前用于委派任务和项目开发任务。记录只追加、不提供更新/覆盖接口；列表按创建时间及时间可排序 GLID 双重倒序，保证同秒写入仍稳定展示最新记录。通知幂等键使用进度记录 ID，连续多次更新不会互相覆盖。
