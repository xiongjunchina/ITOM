...
Task 6 建立了通用 L3 动作边界：预览只信任服务端注册能力及其 Pydantic 输入/固定处理器，在递归脱敏后持久化稳定参数摘要。幂等范围为账号+能力+幂等键：同摘要重试返回原动作，异摘要冲突。确认凭证只存 SHA-256 散列、十分钟过期且一次消费；确认先 `FOR UPDATE` 锁动作，再重载当前账号/角色/权限/发布档案并调用处理器记录级再授权。成功的业务变更、`AiAction` 结果及通用审计同事务提交；业务失败先回滚，再以脱敏、有界错误单独记录 `failed`，处理器直接 `commit/rollback` 会失败关闭。取消、确认和重放均受所属用户和终态约束。SQLite 已覆盖确定性行锁调用顺序与并发不变式；PostgreSQL 双会话确认同一动作只执行一次的真实竞争证据留待 Task 9 IDC 验收。
