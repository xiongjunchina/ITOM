...
网页智能体采用双入口、统一能力内核：网页使用当前 ITOM 登录会话，Aily 保持现有 JWT/MCP 身份，两者只复用下层领域服务和业务约束。WA0 Task 1–4 已实现默认关闭的持久化与 `admin_ai` 权限基础、固定能力注册/请求级策略/递归脱敏、安全 OpenAI-compatible 模型网关，以及仅限 `admin_ai` 的提供商/四类固定档案管理 API。每次提供商请求把公网 DNS 校验结果绑定为实际拨号 IP，同时保留原主机的 Host、TLS SNI 与证书校验；禁环境代理/重定向并严格终止流。认证基础、streaming、强制工具和 strict JSON Schema 四项能力独立精确探测，探测状态原子落库且失败不能保留启用或健康标记。密钥只写加密，档案只能收窄代码注册能力，采用乐观锁、不可变递增版本和复制式回滚；健康、用量和动作审计只返回聚合/脱敏白名单。purpose 只用服务端枚举；主备调用审计使用独立事务，成功审计提交前不发送 `done`，审计失败不提交调用方事务、不掩盖取消或破坏安全回退。管理员 UI、网页会话、动作编排和业务处理器仍待后续任务实现。普通业务用户网页智能体仍只处理本人服务请求，BDO 增加本人 IT 需求；IT 员工按实际权限和流程任务获得全模块指导及分阶段写操作。模型、提示词和管理员档案配置均不是授权来源。现有“创建单据指引”保留为模型不可用时的确定性降级。正式设计见 [`docs/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md)。
