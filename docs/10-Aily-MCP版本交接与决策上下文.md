...
> 状态：**正式设计基线；P0/P1 已完成；P2 代码、自动化、真实 Aily 对话闭环及机器人真实收件已分别通过，普通用户同单端到端 UAT 待完成**
> 确认日期：2026-07-29
...
本文同时记录目标契约与实际状态。当前分支已实现 P0 的协议/身份/消息底座、P1 的服务请求与 IT 需求入口，以及 P2 的受理/待确认打点、3 个闭环 MCP 工具、网页/MCP 共用确认语义、可靠用户消息和评价明细；P2 的真实对话闭环与机器人收件已分别通过，普通用户同一张新工单的端到端 UAT 与 P3 尚未完成。能力状态必须继续以当前分支的真实模型、路由、测试和 Git 记录为准。
...
- 配置 `card_action_skill_id` 后，解决/提醒改发“已解决并关闭、仍未解决”交互卡片，关闭改发 1–5 星卡片；未配置时安全回退纯文本。按钮只触发 Aily Skill，业务动作仍经 MCP；
