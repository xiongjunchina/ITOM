...
2026-08-27 软件版本中心 B1：新增 Git 控制的只读 SemVer 发布清单、公开版本摘要、关于页面和镜像 OCI 版本标签；IDC 发布探针后续必须同时校验健康接口产品版本。此项不修改 MCP `serverInfo`、工具、JWT/Origin/租户/Agent 校验、身份映射、领域服务、RBAC 或审计边界，也不新增数据库迁移。公开版本 API 不暴露 Git SHA、镜像或数据库信息。
