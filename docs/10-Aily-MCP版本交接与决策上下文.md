...
### 3.5 ITOM 网页智能体（设计已确认；WA0 Task 1–5 已实现）

Task 5 会话运行时只接受可证明完整的 schema 标记版本快照、双语提示、合法能力/风险、健康兼容提供商及与活动行一致的最新发布版本；未发布、停用、删除或不一致均失败关闭。普通消息保留期只从创建时捕获的版本读取，绝不由活动档案或 `expires_at` 推断：捕获为 0 时后续正值再发布也不保存正文，捕获为 1–90 时保持原决定和创建时 `expires_at`，但当前档案撤销或受众不一致时停止新正文。所有正文先递归脱敏。`fallback_available` 只反映认证用户既有、权限感知“创建单据指引”的安全载荷可用性，不代表 WA1；列表页码限制为 1–10,000。

网页智能体采用双入口、统一能力内核：网页使用当前 ITOM 登录会话，Aily 保持现有 JWT/MCP 身份，两者只复用下层领域服务和业务约束。WA0 Task 1–5 已实现默认关闭的持久化与 `admin_ai` 权限基础、固定能力注册/请求级策略/递归脱敏、安全 OpenAI-compatible 模型网关、仅限 `admin_ai` 的提供商/四类固定档案管理 API，以及当前登录用户的 bootstrap、创建、列表、详情和归档会话 API。会话所有读取/归档均按数据库 `auth_user_id` 过滤，非归属 ID 使用相同 404；浏览器上下文只接受规范化站内路径、有限标识和最多 20 个 GLID，拒绝客户端角色/权限、DOM/HTML、提示词、Cookie/头、外部/协议相对/穿越式路径和额外字段。档案未发布/停用失败关闭；保留期 0 不保存普通消息正文，1–90 天设置 `expires_at` 且消息先递归脱敏；归档不删除 `ai_action` 或安全/业务审计。每次提供商请求把公网 DNS 校验结果绑定为实际拨号 IP，同时保留原主机的 Host、TLS SNI 与证书校验；禁环境代理/重定向并严格终止流。认证基础、streaming、强制工具和 strict JSON Schema 四项能力独立精确探测；管理探测只在前后两个短事务持 PostgreSQL advisory/行锁，中间网络 await 不持数据库事务，并以 `config_revision` 丢弃变化/删除后的旧结果，失败不能保留启用或健康标记。其他提供商治理及档案发布活动引用仍由同一跨 Pod 锁串行化并按 ID 锁行；密钥只写加密且不参与明文散列。档案只能收窄代码注册能力，所有发布控制字段先进入带 schema 标记的完整版本草稿，成功发布才原子应用活动配置并生成不可变递增版本；复制式回滚只接受可证明完整的历史快照，legacy `{}`/不完整版本失败关闭且绝不以当前活动设置补齐历史。健康和动作审计只返回聚合/脱敏白名单，用量在 1–90 天受控窗口内由 SQL 聚合。purpose 只用服务端枚举；主备调用审计使用独立事务，成功审计提交前不发送 `done`，审计失败不提交调用方事务、不掩盖取消或破坏安全回退。管理员 UI、消息 SSE/工具循环、动作编排和业务处理器仍待后续任务实现。普通业务用户网页智能体仍只处理本人服务请求，BDO 增加本人 IT 需求；IT 员工按实际权限和流程任务获得全模块指导及分阶段写操作。模型、提示词和管理员档案配置均不是授权来源。现有“创建单据指引”保留为模型不可用时的确定性降级。正式设计见 [`docs/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md)。
