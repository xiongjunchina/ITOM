...
- Every table that exceeds the content width uses only Ant Design's native, operable bottom horizontal scrollbar. ITOM no longer creates, synchronizes, or hides a second global floating scrollbar, so off-screen columns remain reachable in every initialization state. The WBS table additionally freezes the first three columns.
