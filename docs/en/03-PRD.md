...
The Requirement Development tab provides **Download Template** and **Bulk Import**. Its template contains requirement code, task name, description, assignee, planned completion date, planned/actual effort, and task status. It accepts `.xlsx` files up to 5MB. Import only appends to real, non-example, non-project-converted requirements in Implementation; the assignee must exactly match an active digital-team member name. Rows are validated independently and return row numbers/reasons: valid rows are created while invalid rows do not block other valid rows, and no existing task is updated or overwritten.
