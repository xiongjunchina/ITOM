...
- ITOM notifications entering the unified `notifier.notify()` outlet now support both in-app delivery and reliable Feishu-bot text delivery. Notification preferences still filter `work`, `workflow`, and `system`; a recipient must have an active verified Feishu identity mapping. A stable digest of event, entity, and recipient identity forms the idempotency key. The business transaction writes `notification_outbox(channel=feishu_aily)`, and a background worker sends asynchronously with exponential retry. Incomplete bot configuration preserves `pending` rows without consuming attempts. Service-request acceptance, resolution, reopen, closure, and rating still use dedicated subscriptions for interactive cards or text fallback; generic delivery skips `ticket.resolved` to avoid duplicating the resolution card. MCP itself does not wake proactively.
