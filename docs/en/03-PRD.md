...
- **Controlled batch deletion**: service requests/changes/incidents/problems, IT requirements, Bugs, requirement-development/project-development/delegated tasks, training activities, CMDB configuration items and relations, and service catalogs/items may select and request deletion of up to 100 records from their own lists. The server evaluates every item through its existing permission, state, reference, and audit guard; no cascade bypass is available. A catalog still containing service items or a service item referenced by an active ticket is rejected. A request may partially succeed and the UI must show both deleted count and per-record rejection reasons.
...
- The wide WBS table provides an Excel-style freeze pane: the header remains visible during vertical scrolling and the first three columns remain visible during horizontal scrolling. It uses the shared browser-native floating bottom scrollbar and retains the native in-table scrollbar as a fallback. Users can drag header separators to resize columns and row edges to resize rows; the layout is saved per project in the current browser.
...
- A confirmed completed WBS task cannot be deleted, re-parented, or reordered. Only an unstarted task (0% progress with no actual start/end record) and its fully unstarted subtree may use **Adjust hierarchy** to become a top-level task or a child of another unstarted parent, and may be drag-sorted among siblings. The server atomically recalculates affected sibling order and WBS codes, retains audit facts, and rejects cycles.
