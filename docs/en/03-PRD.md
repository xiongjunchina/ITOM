...
WA0 Task 6 implements the generic L3 action boundary. Preparation normalizes input only through the code-registered Pydantic model, with stable idempotency scoped to real account + capability + key; same-key/same-input returns existing state and same-key/different-input is rejected. Preview comes only from the fixed server handler. Model/client claims about capability, risk, role, handler, SLA, queue, workflow, assignment, or final state are never authoritative. The raw confirmation token is returned only in the first response and only its SHA-256 is stored; it binds the action, account, conversation, capability, normalized digest, and ten-minute expiry. The owner uses `POST /api/assistant/actions/{id}/confirm|cancel`. Confirmation locks `ai_action`, recomputes current capability, and has the fixed handler recheck data scope, state, ownership, and workflow assignment immediately before mutation. Domain change, `AiAction.succeeded`, result entity, and generic audit commit atomically, and a handler cannot commit or roll back independently. Failure rolls back business state first, then separately stores a redacted failure fact with no raw payload/token/exception body; failure text cannot claim creation, closure, or success. Cancellation, expiry, wrong token, replay, revoked permission, state/assignee drift, and auditor-only writes fail closed. Task 6 registers no concrete business capability; administration UI, message SSE/tool loop, and domain handlers remain pending. Real PostgreSQL two-session contention on the confirmation row is deferred to Task 9 IDC acceptance; SQLite currently proves the deterministic `FOR UPDATE` call and transaction result only.
