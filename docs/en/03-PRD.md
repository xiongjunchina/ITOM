...
WA0 Task 6 implements the generic L3 action boundary. Preparation normalizes input only through the code-registered Pydantic model; if recursive redaction would change any normalized value, the request is uniformly rejected without consuming its idempotency key, and only unchanged safe input is hashed or persisted. Idempotency is real account + capability + key. A PostgreSQL race recovers only the winner of named constraint `uq_ai_action_user_capability_idempotency`: same-key/same-input returns existing state without another token, same-key/different-input conflicts, and unrelated integrity errors propagate. Every fixed L3 handler implements `authorize_preview`, `preview`, and `authorize_record`. Non-leaking record authorization runs before metadata, then an independent Session that is always rolled back and closed produces only an exact `status=prepared` preview through a preview-local actor context and a `ReadOnlyActionData` facade that materializes bounded SELECT reads without exposing raw Session/Connection/transaction surfaces; PostgreSQL also sets transaction read-only and handler write/DML/flush/commit/rollback attempts fail closed. After preview, preparation `FOR UPDATE + populate_existing` locks and revalidates the owned active conversation before action insert or idempotency-winner handling, so archiving or binding changes cannot overtake preview. The raw confirmation token is returned only in the first response and only its SHA-256 is stored; it binds action, account, conversation, capability, normalized digest, and a ten-minute lifetime. The owner uses `POST /api/assistant/actions/{id}/confirm|cancel`. Confirmation first row-locks `ai_action`, then locks and refreshes the owned active conversation before entering the Task 4 governance advisory/provider/profile lock order to prove that the conversation-captured profile remains the complete current publication and its provider remains enabled, healthy, and compatible. It then recomputes capability and calls record authorization immediately before mutation only through an `ActionUnitOfWork` facade with bounded ORM access and no raw commit/rollback/begin/get_transaction/connection/bind surface. Domain mutation, `AiAction.succeeded`, result entity, and generic audit form one success transaction inside a nested savepoint; handlers cannot commit, roll back, or flush. Handler/audit failure rolls back only that savepoint and commits a bounded `failed` fact on the same row while retaining the outer row lock. If failure-state commit itself fails, a bounded persistence error is surfaced instead of claiming a durable terminal state. Cancellation, expiry, wrong token, replay, revoked permission, invalid profile/version/provider, changed state/assignee, and auditor-only write all fail closed. Task 6 registers no concrete business capability; administration UI, message SSE/tool loop, and domain handlers remain pending. Current SQLite automation proves only call order, archive-vs-prepare/archive-vs-confirm ordering, rollback outcomes, injected-race handling, and savepoint semantics. Task 9 IDC must still capture real PostgreSQL read-only preview, same/different-payload prepare races, archive ordering, and confirmation-failure evidence showing a waiting confirmer never invokes the handler twice.
