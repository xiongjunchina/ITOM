...
WA0 Task 4 provides `admin_ai`-only APIs for providers and four fixed profiles. Secrets are encrypted write-only and a blank update preserves existing ciphertext; recent truthful probe health gates provider enablement and profile publication. Profiles may select only code-registered capabilities under fixed audience, risk, and knowledge-scope ceilings, with optimistic draft locking, immutable increasing published versions, and copy-based rollback. Health, usage, and action-audit reads are aggregate/redacted allowlists only. The administration UI, web conversations, action orchestration, and business capabilities remain pending.
