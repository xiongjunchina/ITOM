...
WA0 Task 7 implements guarded SSE orchestration at `POST /api/assistant/conversations/{id}/messages`. The request accepts only text, a client-message idempotency key, and Task 5's allowlisted page context. Events are fixed to `meta|delta|message|action|error|done`; a completed turn has exactly one `done`, errors cannot be followed by success, and one-line JSON encoding prevents SSE injection. Prompt layers keep the platform security instruction, immutable published profile, per-call authorized capability schemas, and explicitly untrusted page/knowledge/business/user bodies separate; browser roles, permissions, and tool results never become system authority. Every tool call resolves a fixed registry code, re-authorizes, and validates through the registered Pydantic model. Model-supplied handler/risk/role/result, unknown or disabled codes, invalid arguments, repeated calls, and more than four rounds fail closed. L3 calls Task 6 `prepare_action()` only and emits the server preview/token without confirming or executing; a model success claim is blocked when no committed server result exists. Event, token, text, and turn-time budgets are bounded. Provider outage, timeout, malformed/truncated protocol, and client disconnect cancel remaining work and return a secret-free safe error plus deterministic native path. Only a redacted final answer becomes `completed` under captured retention; partial/provider-error/secret text never does, while retention zero stores bodyless idempotency metadata only. The same `client_message_id` safely replays equal input and conflicts on different input; cross-user conversation access is uniformly denied. Task 7 registers no concrete domain capability, implements no frontend/administration UI, and does not weaken Task 6 confirmation atomicity.
...
| Web Agent WA0–WA4 (design approved; WA0 Tasks 1–7 implemented, Task 8+ pending) | 2, 3, 5–10, 11 | Persistence, fixed capability/live policy/redaction, the secure model gateway, provider/profile administration APIs, owner-scoped web-conversation lifecycle, and generic L3 server preview/confirmation/idempotency/reauthorization/atomic audit are implemented; administration UI, SSE/tool loop, role-aware read-only advisor, concrete business capabilities, requester service-request and BDO requirement loops, controlled IT-staff writes, and quality governance continue by phase; ITOM permission/domain services remain authoritative |
