...
### 5.1a IT-staff routing, record guidance, and cross-record relations (phases A/B implemented; create-and-relate pending)
...
Every escalation or transfer follows “**create target + relation + retain source**”: service request → incident (`upgraded_to_incident`), service request/incident → problem (`root_cause_of`), incident/problem → change (`remediated_by_change`), and IT requirement → project (`converted_to_project`). Phase B now delivers the generic relation's additive model, immutable audit, idempotency, two-sided visibility filtering, and bidirectional relation display in ticket, problem, requirement, and project details, without migrating or overwriting dedicated legacy links. Phase C will provide the four submit paths that create the target and the audited relation in one transaction only after target-domain required-field, permission, workflow, and approval checks pass. A source record is never retyped, deleted, force-closed, or overwritten. See `docs/en/superpowers/specs/2026-07-31-it-staff-intake-and-record-relations-design.md`.
...
The phase-C “convert to project” path must use `record_relation(relation_type=converted_to_project)` for unified audit. The project is still created through the project domain service and starts its own workflow; the source requirement is not retyped or automatically closed.
