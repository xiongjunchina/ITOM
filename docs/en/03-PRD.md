...
**Resizable list columns**: every visible shared-list header separator supports mouse dragging and keyboard left/right adjustment. The resize cursor appears only when the mouse approaches the separator; no prominent permanent visual handle is used. Fields without a predefined width also receive a resizable default, and a width change must be applied to the actual table layout. Column width and visibility preferences are isolated by current account, route, and column set so multiple tables on the same page cannot overwrite one another. Purpose-built wide tables such as WBS retain their specialized resize implementation and must not be overridden by the shared header component.
...
- Every table that exceeds the content width uses only Ant Design's native horizontal scrollbar kept at the bottom of the current visible area. It remains usable before the user reaches the final list row, so off-screen columns stay reachable. ITOM no longer creates, synchronizes, or hides a second global floating scrollbar. The WBS table additionally freezes the first three columns.
