...
### 10.1 ITOM Web Agent (design approved; WA0 Tasks 1–7 implemented)
...
| Web Agent WA0–WA4 (design approved; WA0 Tasks 1–7 implemented, Task 8+ pending) | 2, 3, 5–10, 11 | Persistence, fixed capability/live policy/redaction, the secure model gateway, provider/profile administration APIs, owner-scoped web-conversation lifecycle, generic L3 server preview/confirmation/idempotency/reauthorization/atomic audit, and the guarded POST-SSE/tool loop are implemented; administration UI, role-aware read-only advisor, concrete business capabilities, requester service-request and BDO requirement loops, controlled IT-staff writes, and quality governance continue by phase; ITOM permission/domain services remain authoritative |
