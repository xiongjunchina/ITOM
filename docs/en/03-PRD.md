...
**Sticky list headers, horizontal access, and resizable columns**: on primary pages, shared-list sticky headers and Ant Design's single horizontal scrollbar must both bind to the real `.app-content` vertical scroll container. The header remains visible while a long list scrolls vertically, and the horizontal scrollbar remains at the bottom of the current viewport without a second global floating scrollbar. Every visible header separator supports mouse dragging and keyboard left/right adjustment. The resize cursor appears only when the mouse approaches the separator; no prominent permanent visual handle is used. Fields without a predefined width also receive a resizable default, and a width change must be applied to the actual table layout. Column width and visibility preferences are isolated by current account, route, and column set so multiple tables on the same page cannot overwrite one another. Purpose-built wide tables such as WBS retain their specialized resize implementation and must not be overridden by the shared header component.
...
- The wide WBS table provides an Excel-style freeze pane: the header remains visible during vertical scrolling and the first three columns remain visible during horizontal scrolling. Ant Design's native bottom horizontal scrollbar remains available. Users can drag header separators to resize columns and row edges to resize rows; the layout is saved per project in the current browser.
- Every primary-page business list keeps its header visible at the top of the application content viewport while the list scrolls vertically. Every table that exceeds the content width uses only Ant Design's native horizontal scrollbar kept at the bottom of the current visible area, so off-screen columns stay reachable before the final row. Header and scrollbar both use `.app-content` as their positioning container. ITOM no longer creates, synchronizes, or hides a second global floating scrollbar. The WBS table additionally freezes the first three columns.
