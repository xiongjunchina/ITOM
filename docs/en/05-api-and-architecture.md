...
- Corrective tasks must first create a local lifecycle record through `scripts/task-lifecycle.py` under `skills/itom-fix-delivery/SKILL.md`. Focused real-target acceptance gates formal documentation and commit; a second failure of the same acceptance target requires root-cause review; and a frozen candidate permits only one complete CI and one IDC release attempt rather than an unchanged retry. The ledger stores no business data or secrets and changes no API, domain model, or IDC data; it exists only for delivery gates, segmented timing, and weekly metrics.
