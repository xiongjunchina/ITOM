...
MCP cannot wake Aily on a background transition. First acceptance, resolution, reopen, closure, and rating-save events write `notification_outbox(channel=feishu_aily)` and a worker sends through the Feishu bot. Once bot credentials, message enablement, the card-callback Verification Token, and the Encrypt Key are all ready, resolution/deadline reminders become close/reopen cards and closure notifications become 1–5-star cards; otherwise delivery remains text-compatible. The first unresolved click only replaces the card with a required reason form, and submission then reopens the request.
...
WA0 Task 7 adds `POST /api/assistant/conversations/{id}/messages` (SSE) and `AssistantOrchestrator.stream_turn()`. The request accepts only `content`, `client_message_id`, and Task 5's allowlisted `page_context`. The router passes only an immutable account ID and ends the request-dependency transaction before streaming network waits. Start, capability discovery, each tool, L3 preparation, and final persistence use separate short Sessions; `_TurnState` contains scalar snapshots only and never retains ORM across provider awaits. The default Gateway selects and detaches provider configuration in a short Session, releases its transaction/connection, and only then begins the up-to-65-second network stream. Event types are fixed to `meta|delta|message|action|error|done`; each `data` value is one-line JSON. Headers are `Cache-Control: no-store, private`, `Vary: Authorization`, and `X-Accel-Buffering: no`. A normal path has exactly one `done`, and no success event may follow `error`.

Platform security instruction, immutable published profile, per-call authorized capability schemas, and explicitly untrusted knowledge/business/page-context/user bodies remain separate layers; browser-claimed role, permission, and tool result never become authority. Output and redacted platform/profile text first undergo Unicode NFKC, casefold, and removal of every Unicode format/zero-width character. Stable line, sentence, and sliding fingerprints are then built for both a semantic-boundary form and a punctuation/symbol-free compact form: complete normalized lines of at least 12 characters, strong fragments of at least 24 characters, or two distinct 12–23-character fragments fail closed; short common words do not participate. Every tool call resolves a fixed registry code and re-authorizes through the registered Pydantic input model. Model-supplied handler/risk/role/result, unknown or disabled code, invalid arguments, equal repeated calls, and more than four rounds fail closed. L1/L2 handlers receive no SQLAlchemy Session, ORM `AuthUser`, Connection, or Engine; they receive Task 6 `ReadOnlyActionData`, immutable `ActionActorContext`, and a cooperative `CapabilityExecutionContext` only. A dedicated bounded executor accepts 1–32 workers and 0–256 queued calls and rejects saturation before creating a tool Session. Total tool deadline is restricted to 0.1–60 seconds; PostgreSQL `statement_timeout` is 10–59,000 ms and must be strictly shorter. PostgreSQL executes `SET TRANSACTION READ ONLY` before any read, then the tool Session always rolls back/closes. The facade denies commit/flush/rollback/add/delete/merge/query/raw SQL/Core DML/ORM mutation. Disconnect/deadline makes the event loop stop waiting immediately, emits no more events, and signals cooperative cancellation. A synchronous Python thread cannot be safely force-killed: a non-cooperative block may continue in the background until it returns, but its Session still closes; hard termination would require process isolation. L3 may finish as `prepared` until expiry but can never execute a mutation. Event, text, token, tool, and total-turn budgets are hard bounded.

During streaming, L3 may call Task 6 `prepare_action()` only and emit server action ID, risk, preview, one-time confirmation token, and expiry; it never confirms or executes. Its final message is server-rendered as `authority=server_preview, operation_status=prepared_not_executed`, and model success prose is discarded. Without a committed server result, normal model prose is persisted only as non-authoritative `authority=advisory, operation_status=not_executed`; it is not an ITOM success or status result. Provider outage, timeout, malformed/truncated protocol, illegal tool output, or disconnect stops waiting and returns a uniform non-leaking error. The deterministic native fallback is the first current-account `can_create` target computed by `it_document_guide.guide_payload()`, or `/` when none is authorized. Final completion uses a fresh short transaction and a fixed lock order: first `SELECT ... FOR UPDATE + populate_existing` the `AuthUser` and validate active/deleted after locking, then lock the owned active conversation, exact current published profile/version, and still-`streaming` assistant placeholder. Archive, account disablement, profile withdrawal/replacement, missing placeholder, or status conflict fails closed with no success message/done. Only a redacted final message may persist under captured retention; partial deltas, provider errors, and secrets never become completed answers, while retention zero stores bodyless idempotency metadata. `request_digest` is a server-key HMAC over normalized raw `content + page_context`; an equal raw request safely replays the same `client_message_id`, while a raw mismatch conflicts even if redaction produces the same body. SQLite automation verifies query order and lock markers; real PostgreSQL account/conversation/profile lock waits remain Task 9 evidence.
...
| Web Agent WA0 (Tasks 1–7 implemented; Task 8+ pending) / WA1–WA4 | WA0 persistence, fixed capability registry, live role policy, recursive redaction, secure OpenAI-compatible model gateway, provider/profile administration APIs, owned web-conversation lifecycle, generic L3 server preview/confirmation boundary, and guarded POST-SSE/tool loop; concrete domain capabilities remain pending | global assistant, structured cards, and AI Assistant administration UI pending | web-agent design baseline |
