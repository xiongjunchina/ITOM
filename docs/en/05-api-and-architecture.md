...
- Every business table enters the unified kernel through `SortableTable` or `StickyTable`. `ResponsiveTableEnhancer` gives only the active wide table intersecting `.app-content` one body-level native floating scrollbar synchronized with the real `overflow-x:auto` body. `SortableTable` defaults the first two non-action business leaves to `fixed:left`; a row-selection column is fixed separately, while explicit fixed columns and WBS's specialized first-three rule take precedence. Column settings protect those context fields. Ordinary lists use `table-layout:fixed`. Automatic columns first estimate a first-frame width from header/current-row text, then clone real header/body cells into a hidden `width:max-content` host, explicitly copy horizontal padding, and take the largest rendered width per column. Automatic cells disable ellipsis and remain `nowrap`; an empty column is still sized by its header. `scroll.x` is exactly the displayed-width sum, with neither a historical page floor nor spare viewport stretching; unified tables override the short-table `min-width:100%` so unused viewport space remains outside the table. Resize logic updates every same-index sticky/ordinary header and body `col`, adjusts following fixed offsets, then adds the field to `manual_widths` and persists once. Restore Defaults clears manual markers; migration ignores legacy all-column default widths unless they differ from the old declared width. Action width remains 64–176px from the maximum visible rendered control count. Opaque fixed cells use isolated stacking and edge shadows. `StickyTable` owns no second bottom scrollbar.
- `python -m app.scripts.seed_table_uat seed|cleanup --confirm-local` is an explicit, non-startup `feature-local` acceptance utility, not an HTTP API, migration, or production seed. `assert_local_uat_database()` fails closed for databases other than SQLite, the local machine, or the repository Compose service, after which the command idempotently creates or soft-deletes wholly fictitious editable records carrying dedicated `UAT-TABLE` codes and the `【本地表格UAT】` marker and explicitly commits the successful local transaction. It prints entity counts only, never connection credentials or secrets, and deployment/startup/IDC flows must never call it.
