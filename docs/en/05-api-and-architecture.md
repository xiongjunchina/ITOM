...
- `deploy/k8s/k8s-deploy.sh` deploys that same tag while preserving existing Secrets, PVCs, database, uploads, and Feishu configuration. An approved non-database application recovery may use `SKIP_DATABASE=1 ./k8s-deploy.sh`: it skips applying `10-postgres.yaml` and waiting for the PostgreSQL StatefulSet rollout, and never deletes, restarts, or reschedules the database Pod/PVC. That mode is prohibited for schema-affecting releases, which still require the approved in-cluster backup/checkpoint first. The current IDC frontend policy permits only `sn-prod-k8s-01` and `sn-prod-k8s-02`: the manifest explicitly uses an empty `nodeName` to clear a manual recovery pin, and required hostname pod anti-affinity spreads the two replicas across those normal eligible nodes. Node 03 does not host ITOM frontend. Frontend rolling updates use `maxSurge: 0` and `maxUnavailable: 1`, releasing one eligible host before creating its replacement so required anti-affinity cannot deadlock when only two hosts are eligible. The backend remains a single unpinned `Recreate` Deployment because its uploads PVC is RWO; the database StatefulSet, PVCs, and backup Job are outside application redeployment scope. The script fails on rollout, Ready Endpoint, image identity, the in-cluster backend proxy of **every** Ready frontend endpoint, external `/api/health`, or MCP `initialize` errors.
