...
- After the gate passes, `deploy/k8s/push-images.sh` accepts only a clean commit; obtains Python, Node, and Nginx from the verified `mirror.gcr.io` Docker Library cache at pinned digests; builds and verifies linux/amd64 images; applies the default immutable `git-<first-12-commit-chars>-linux-amd64` tag; and pushes to Harbor. Image building does not start a local ITOM runtime or depend on anonymous Docker Hub limits.
