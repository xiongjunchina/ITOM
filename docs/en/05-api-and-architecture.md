...
WA0 Task 3 implements the provider-neutral `ModelProvider` contract, an OpenAI-compatible `/chat/completions` adapter, and `AssistantGateway`. `AI_PROVIDER_ALLOWED_HOSTS` must contain comma-separated exact hosts or explicit `*.controlled-suffix` entries; an empty allowlist fails closed. Base URLs require HTTPS and reject URL credentials, query, fragment, and path escape. DNS is resolved again immediately before every request, and loopback, private, link-local, multicast, unspecified, reserved, and metadata-class addresses are denied. The HTTP client ignores environment proxies and never follows redirects. `AI_PROVIDER_CONNECT_TIMEOUT_SECONDS` defaults to 5 and `AI_PROVIDER_READ_TIMEOUT_SECONDS` defaults to 60. Streaming accepts only validated SSE deltas, complete JSON-object tool arguments, usage, supported `stop/tool_calls` terminal reasons, and final `[DONE]`; unknown events, malformed JSON, truncation, or missing terminal semantics fail closed. The gateway uses enabled, recently successful, policy-compatible primary/fallback providers only; L2/L3 require both tool and JSON Schema support, and fallback is forbidden after primary output begins. Each real attempt writes only provider, model, purpose, tokens, latency, result code, status, and redacted error to `ai_provider_call`; prompts, response bodies, and secrets are never stored. Task 3 adds no admin API, `/api/assistant` router, UI, conversation/action orchestration, or business handler.
...
| Web Agent WA0 (Tasks 1–3 implemented; Task 4+ pending) / WA1–WA4 | WA0 persistence, fixed capability registry, live role policy, recursive redaction, and the secure OpenAI-compatible model gateway; administration API, conversation routing, and domain-service reuse remain pending | global assistant, structured cards, AI Assistant administration pending | web-agent design baseline |
