...
| Web Agent WA0 (Tasks 1–6 implemented; Task 7+ pending) / WA1–WA4 | WA0 persistence, fixed capability registry, live role policy, recursive redaction, secure OpenAI-compatible model gateway, provider/profile administration APIs, owned web-conversation lifecycle, and generic L3 server preview/confirmation/idempotency/reauthorization/atomic audit; SSE/tool loop and concrete domain capabilities remain pending | global assistant, structured cards, and AI Assistant administration UI pending | web-agent design baseline |
