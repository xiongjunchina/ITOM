...
- Before modification, every coding task uses the repository-level `.agents/skills/itom-task-routing` skill to confirm `production-fix|feature-local|code-candidate`, then reads the matching `itom-fix-delivery|itom-feature-delivery|itom-code-candidate-delivery` skill and creates a local lifecycle record through `scripts/task-lifecycle.py --track`. `production-fix` defaults to real IDC fault evidence without a local application runtime. `feature-local` may use the repository-defined isolated Docker stack and test database to reach `local_candidate_ready`; production data, credentials, Secrets, OAuth/Aily applications, and callbacks remain forbidden, and the containers, database, volumes, ports, and acceptance data must stay available until the user explicitly completes UAT or requests shutdown. `code-candidate` must not start, stop, or alter an application runtime and cannot enter IDC states. Focused real-target acceptance gates formal documentation and commit, and a second failure of the same target requires root-cause review. Every IDC write also requires a separate post-CI presentation of the commit, immutable tag, affected objects, database impact, disruption, rollback, and acceptance method, followed by explicit user authorization recorded through `approve-idc`; confirmation that local UAT passed is not itself that authorization.
