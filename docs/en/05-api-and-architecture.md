...
MCP cannot wake Aily on a background transition. ITOM notifications entering `notifier.notify()` write a `notification_outbox(channel=feishu_aily)` text row in the original business transaction for each recipient with an active mapped Feishu identity. A stable event/entity/identity digest provides idempotency; a worker sends asynchronously with exponential backoff and leaves rows pending while bot configuration is incomplete. First acceptance, resolution, reopen, closure, and rating-save events still use dedicated subscriptions for interactive cards or text fallback; generic delivery skips `ticket.resolved` so a resolution card is not duplicated. The first unresolved click only replaces the card with a required reason form, and submission then reopens the request.
...
Delivery keeps event/entity/identity idempotency, retry/backoff, and redacted errors. Disabled or incomplete bot configuration preserves pending rows without consuming attempts. The most recently used active Feishu identity is selected, and internal notes, root cause, approval details, and sensitive fields never leave ITOM. Administrators use the existing `GET/PUT /api/admin/integrations/aily` configuration for the message switch, bot credentials, tenant allowlist, and public base URL; secrets remain write-only and no new API or database migration is required.
...
IDC Kubernetes:
  ...
  frontend:  nginx serving the build output and proxying /api and /mcp; two replicas with soft hostname topology spread
```
...
- `deploy/k8s/k8s-deploy.sh` deploys that same tag while preserving existing Secrets, PVCs, database, uploads, and Feishu configuration. Frontend replicas use soft `kubernetes.io/hostname` topology spread (`maxSkew=1`, `ScheduleAnyway`) rather than a single-node pin: they prefer separate healthy nodes and can still recover on one node during reduced capacity. The script fails on rollout, Ready Endpoint, image identity, the in-cluster backend proxy of **every** Ready frontend endpoint, external `/api/health`, or MCP `initialize` errors. Schema-affecting releases require the approved in-cluster backup/checkpoint first.
