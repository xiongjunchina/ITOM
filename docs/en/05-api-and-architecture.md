...
- `deploy/k8s/k8s-deploy.sh` deploys that same tag while preserving existing Secrets, PVCs, database, uploads, and Feishu configuration. The current IDC frontend policy permits only `sn-prod-k8s-01` and `sn-prod-k8s-02`: only ITOM frontend explicitly tolerates node 02's `isa.io/build-executor=true:NoSchedule` taint, and required hostname pod anti-affinity spreads the two replicas across the eligible nodes. Node 03 no longer hosts ITOM frontend. The backend remains pinned to node 02; the database StatefulSet, PVCs, and backup Job are outside application redeployment scope. The script fails on rollout, Ready Endpoint, image identity, the in-cluster backend proxy of **every** Ready frontend endpoint, external `/api/health`, or MCP `initialize` errors. Schema-affecting releases require the approved in-cluster backup/checkpoint first.
