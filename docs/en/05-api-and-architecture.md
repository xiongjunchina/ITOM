...
WA0 Task 7 adds `POST /api/assistant/conversations/{id}/messages` (SSE) and `AssistantOrchestrator.stream_turn()`. The request strictly accepts only `content`, `client_message_id`, and Task 5's allowlisted `page_context`; the response generator opens its own database Session and reloads the current account so no detached request identity is used after dependency cleanup. Event types are fixed to `meta|delta|message|action|error|done`, and every `data` value is one-line JSON encoded. A normal terminal path has exactly one `done`, while no success event may follow `error`. Platform security instruction, immutable published profile, per-call authorized capability schemas, and explicitly untrusted knowledge/business/page-context/user bodies are separate layers; browser-claimed role, permission, and tool result never become authority. Each tool call resolves a fixed registry code and re-authorizes, with the registered Pydantic model validating arguments. Model-supplied handler/risk/role/result, unknown or disabled code, invalid arguments, repeated equal calls, and more than four rounds fail closed. Events, text, tokens, and total turn time are hard bounded. During streaming, L3 may call Task 6 `prepare_action()` only and emit the server action id, risk, preview, one-time confirmation token, and expiry; it never confirms or executes. Without a committed server result, the model cannot claim success. Provider outage, timeout, malformed/truncated protocol, illegal tool output, or disconnect cancels remaining provider/tool work and returns a uniform error plus deterministic native fallback without raw exception, prompt, system instruction, or secret. Only a redacted final message is persisted under captured conversation retention; partial deltas, provider errors, and secrets never become completed answers, while retention zero stores bodyless idempotency metadata only. Equal input safely replays the same `client_message_id`, different input returns 409, and cross-user conversation access remains uniform 404.
