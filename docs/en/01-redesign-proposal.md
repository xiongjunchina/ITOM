...
| --- | --- |
| Service Request | Open count, P1–P4 priority distribution, resolved this month, SLA attainment rate |
| Change | Open count, P1–P4 priority distribution, pending approval, implementing, success rate |
| Incident | In-progress count, P1–P4 priority distribution, SLA warnings, resolved this month |
| Problem | In-progress count, P1–P4 priority distribution, known errors, closure rate |
...
| --- | --- | --- |
| Performance Management | Implement the matrix-role model in [IT Team Role Performance PRD](08-it-team-role-performance-prd.md): matrix role scoring 80% + team contribution 20%; automatic facts from ITSM/requirements/projects/processes; scoped manager proposals; CIO final review and direct platform-role scoring; external business ratings entered through a dedicated raw-input page. | Delete `cell_performance`(20) / `kpi_snapshot` / `growth_monthly_stats` (all pre-computed tables); replace the old `performance_rule` concept with role profiles, score components, and period snapshots. |
...
2. **Performance scoring rules** — the current implementation baseline is the matrix-role rule set (see [`08-it-team-role-performance-prd.md`](08-it-team-role-performance-prd.md)): 80% matrix-role contribution + 20% team contribution. CIO can configure role profiles, dimension weights, and team-contribution targets; published periods retain versioned snapshots.
