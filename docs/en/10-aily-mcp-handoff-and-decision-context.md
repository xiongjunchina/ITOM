...
Every coding task confirms a `production-fix|feature-local|code-candidate` route before modification and creates lifecycle evidence through `scripts/task-lifecycle.py --track`. Production fixes start from real IDC facts; new features may form a candidate in an isolated local Docker environment containing no production data, credentials, Secrets, OAuth/Aily applications, or callbacks; code candidates start no application runtime. Formal documentation and commit follow focused target acceptance, and a frozen candidate runs complete CI once. Every IDC write separately presents the exact commit, immutable tag, affected objects, data impact, disruption, rollback, and acceptance plan after CI, then records explicit user `approve-idc` authorization. Neither local acceptance nor CI is production delivery.
