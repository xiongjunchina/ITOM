...
> Status: **formal baseline; P0/P1 complete; P2 live same-ticket end-to-end UAT has passed for both text and signed card buttons; P3 Feishu Approval is deferred, while trusted IDC TLS is deployed and protocol-validated and security/performance/recovery plus current-version real-role formal UAT remain**
...
This document records both target contracts and actual status. The branch implements P0 protocol/identity/messaging foundations, P1 service-request and requirement intake, and P2 acceptance/confirmation timestamps, three closure MCP tools, shared web/MCP confirmation semantics, reliable user messages, and rating detail. The real-Aily conversational loop, live bot receipt, normal-user same-ticket text loop, and signed-card-button loop have all passed. The generic notification outlet now also sends ITOM notifications entering `notifier.notify()` through the reliable Feishu text outbox, reusing identity mapping, preference filtering, idempotency, retry, and the existing worker; service-request resolution remains on its dedicated interactive-card path. P3 Feishu Approval is deferred by user decision. IDC is available and the current version plus public protocol probes and trusted-TLS protocol validation are deployed, while security/performance/recovery and current-version formal real-role UAT remain.
...
### 3.5 ITOM web agent (design approved; WA0 Tasks 1–8 implemented)
...
The web agent uses two entrances and one capability core: web uses the current ITOM login, Aily retains its JWT/MCP identity, and both reuse only lower-level domain services and business guards. WA0 Tasks 1–8 implement disabled-by-default persistence and `admin_ai` permission foundations, fixed capability registration/request-time policy/recursive redaction, the secure OpenAI-compatible model gateway, `admin_ai`-only provider/four-profile administration APIs, current-login conversation APIs, the generic L3 preview/confirm/cancel boundary, and a guarded POST-SSE/tool loop whose event set is `meta|delta|message|action|error|done`. Task 7 Fix Round 1 narrowed runtime state to scalar turn snapshots and separate short transactions: L1/L2 receives only a read-only facade plus immutable actor; L3 returns only a server-rendered pending/not-executed preview; normal model prose is `advisory/not_executed`; request idempotency uses a server-key HMAC. Fix Round 2 closes four additional findings: prompt-leak checks apply NFKC/casefold, remove format/zero-width characters, and compare semantic plus punctuation-free compact forms; final persistence first locks/refreshes the account and validates it after locking before conversation/profile/placeholder locks; L1/L2 uses a dedicated bounded executor with cooperative cancellation and saturation rejection before tool Session creation; and tool/database deadlines plus worker/queue settings have safe ranges and cross-field validation. Disconnect/deadline immediately stops waiting and event emission but does not claim to force-kill arbitrary synchronous Python threads. A non-cooperative block may continue until return while its Session still closes; arbitrary hard termination requires process isolation. Conversations/actions remain isolated by database `auth_user_id`; models, prompts, and client claims never authorize, and every tool call re-authorizes a fixed registry code. Existing provider security, profile publication, conversation ownership/retention, recursive redaction, audit atomicity, and fail-closed contracts continue to apply. The administration UI is implemented by Task 8; concrete business handlers remain WA1+ work. A normal business user's web agent remains limited to own service requests; BDO adds own IT requirements; IT staff receive system-wide guidance and progressively enabled writes under actual permissions and process assignment. The existing Record Creation Guide computes an authorized path from current permissions and remains the deterministic fallback. Real PostgreSQL two-Session, account/conversation/profile lock waits, non-cooperative-thread, and real ASGI disconnect evidence remain Task 9. See [`docs/en/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md).

Task 7 Fix Round 3 closes four runtime gaps without adding a business capability: compact leak fingerprints retain only Unicode `L*`/`N*` after NFKC/casefold, so `M*`/`C*`/`Z*`/`P*`/`S*` insertion cannot split matches; tool calls reserve bounded capacity before capability discovery/reauthorization, so saturation creates no Session, reads no permission, and runs no handler; synchronous database boundaries for start/idempotency, discovery, native fallback, Gateway selection/audit, finalization, and failure cleanup move off the async SSE loop; and finalization cooperatively checks disconnect after provider return, after final locks, and before writing/committing `completed`, rolling back observed cancellation. This does not claim thread force-kill or removal of every real-socket/lock-scheduling micro-race; Task 9 still requires real PostgreSQL/ASGI evidence.

Task 7 Fix Round 4 closes the three remaining review findings without entering Task 8. The compact leak path now admits only original `L*`/`N*` code points while excluding named/explicit Hangul `FILLER`, then applies NFKC/casefold and a second `L*`/`N*` filter, preventing original Mn/Sc/So-style compatibility decompositions from being washed into letters; perfect detection of visually confusable different code points is outside the WA0 guarantee. All synchronous assistant DB offload uses bounded executors. Message-stream authentication returns only an account-ID scalar and closes its worker Session before `StreamingResponse`; Gateway selection/audit also avoids the default executor. `stream_turn()` creates one monotonic deadline shared by fallback/start/provider/Gateway/DB/tool/finalization, reserving at most 250ms (25%) for failed-placeholder cleanup. Tool, provider, and statement timeouts take the smaller of their own cap and remaining budget. Round 5 further clarifies that this budget strictly covers pre-commit work while terminal consistency takes precedence during post-authority-commit cleanup. Real PostgreSQL lock waits, dual-Session idempotency, and real ASGI disconnect evidence remain Task 9.

Task 7 Fix Round 5 closes the final three independent review findings without entering Task 8. Production Provider DNS now calls synchronous `socket.getaddrinfo` inside a dedicated bounded DNS/IO executor whose admission and wait share the request absolute deadline; `loop.getaddrinfo`, `asyncio.to_thread`, and the default executor are forbidden while SSRF, DNS-rebinding, and original-host TLS binding remain intact. Disconnect cancellation audit non-blockingly reserves the existing bounded DB executor and runs as background best effort: cancellation propagates immediately, saturation opens no Session, and failures produce only redacted local warnings; normal success and handled-error paths still await durable audit. One absolute deadline strictly bounds every pre-commit stage. After final locks and a last deadline/cancellation check, authority commit may begin; the caller then waits for commit and Session cleanup to keep durable `completed` and the client terminal consistent, so this cleanup may slightly exceed the deadline and the entire return path is no longer described as an absolute hard bound. Real PostgreSQL commit/disconnect critical races, real ASGI sockets, and dual-Session idempotency remain Task 9 evidence.

Task 7A repairs finalization authority without expanding WA0 or entering Task 8. Thread-safe durable success is set only after `db.commit()` returns successfully. A later Session-close failure records only a redacted exception type, never re-queries the database to guess transaction outcome, and cannot turn durable `completed` into `error → done`. If disconnect is observed after commit has started, cancellation wins and no later SSE terminal is emitted even when cleanup also fails; commit failure sets no durable success and retains rollback, failed-placeholder cleanup, and the safe-error path. Real PostgreSQL commit/disconnect critical-race and real ASGI-socket evidence remain Task 9 work.

Task 8 completes the WA0 frontend consumption layer. The business portal and internal workbench share a global top-bar launcher, and the responsive drawer calls the existing bootstrap, conversation, POST-SSE, and action APIs. A strictly bounded parser accepts only the fixed events and rejects unknown, malformed, truncated, post-error success, post-terminal, and oversized data. Page context comes only from route/tab/explicit-GLID allowlists and never from DOM scraping; output and server previews render as text only. L3 cards cover confirmation, cancellation, expiry, conflict, failure, and success states. Only a server confirmation result of `succeeded` is authoritative success; the confirmation token is never displayed and is cleared on terminal state. System Administration adds an `admin_ai`-guarded five-tab console for providers, four fixed profiles, health, usage, and action audit, with write-only secrets that are never prefilled. This task changes no backend, persisted model, concrete domain capability, or deployment. Task 9 retains real PostgreSQL/ASGI/IDC role and action evidence, while WA1+ retains concrete business capabilities.

Independent Task 8B repairs the confirmation bridge between the Task 8 frontend and existing L3 backend without reopening the five-review Task 8 or expanding WA0. The strict state machine now accepts exactly pre-start `error → done(error)`, post-metadata `meta → error → done(error)`, and `meta → message → done(replay)` with either `advisory` or `server_preview` authority. Error after delta/action/message, success data in an error terminal, replay preview carrying action/delta/token, and every other malformed crossover fail closed. A replayed server preview is informational only and creates neither a card nor executable meaning. The first owner-scoped L3 action SSE uses a fixed narrow server projection to deliver one raw confirmation token with a safe preview. Generic redaction is unchanged; only SHA-256 is stored, and the token enters no model, message, persistence, log, audit, or REST response. Exactly one same-owner valid confirmation succeeds; repeat, cross-owner, expired, cancelled, redacted-placeholder, and malformed credentials fail closed. Task 8B adds no field, migration, business capability, deployment, or IDC evidence, and Task 9's deferred scope is unchanged.

Task 8C corrects the WA0 confirmation transport/state boundary without adding a business capability, migration, deployment, or Aily/MCP change. `AiAction.expires_at` remains naive UTC in storage, while public `confirmation_expires_at` and action-SSE `expires_at` are explicit-`Z` RFC 3339 UTC; the browser rejects offset-free date strings. Live and replay `server_preview.action_id` both fail closed against the same ULID grammar. After existing owner, credential, expiry, conversation/profile, and capability reauthorization, confirmation commits a non-confirmable/non-cancellable/non-retryable internal `executing` claim before calling a handler. Known handler/audit failure can still persist `failed`; uncertain handler or terminal persistence after the claim keeps the non-confirmable state and returns safe outcome-unknown, never restoring `prepared`, reissuing a token, or reporting a business success without a successful transaction. Domain mutation, `succeeded`, and audit remain one transaction. Task 9 real PostgreSQL/ASGI/IDC evidence remains pending.

Task 8C Round 1 closes three review findings: a live action SSE with a valid raw token must bind confirmation to a non-null, parseable explicit-`Z` expiry, so missing/`null`/offset-free/malformed expiry fails closed before card creation; a tokenless action remains informational only. SQLite establishes a post-claim outer write transaction before the handler savepoint, so a failed success-terminal commit rolls back domain mutation, success result, and audit while the database retains non-retryable `executing`; a failed claim does not start the handler and leaves the original `prepared` token for an honest retry. When the browser receives `AI_ACTION_OUTCOME_UNKNOWN`, it shows only outcome-pending verification, clears the token, and disables confirm/cancel without displaying any “not executed” or success assertion. Task 9 real PostgreSQL/ASGI/IDC evidence remains pending.
...
IDC Kubernetes is the sole production-delivery and final-acceptance environment at the current public root `https://itom.snnc.cc:30443`, serving the frontend, `/api`, Feishu OAuth callback, and `/mcp/`. Administrators maintain the root through `public_base_url`, from which the page derives every public endpoint. The current IDC frontend policy limits ITOM frontend to nodes 01/02: only ITOM frontend explicitly tolerates node 02's build taint, and required hostname pod anti-affinity distributes the two frontend replicas; node 03 no longer hosts ITOM frontend. Frontend rolling updates use `maxSurge: 0` and `maxUnavailable: 1`, releasing the node occupied by an old replica before creating its replacement when only nodes 01/02 are eligible, so default surge capacity and required anti-affinity cannot deadlock a rollout. The backend remains pinned to node 02, and the database StatefulSet/PVCs and backup Job are outside application redeployment scope. An approved non-database application recovery uses `SKIP_DATABASE=1 ./k8s-deploy.sh`, explicitly skipping PostgreSQL manifest apply and StatefulSet waiting without deleting, restarting, or rescheduling the database Pod/PVC; schema-affecting releases must not use this mode. The repository Compose stack may run only for a confirmed `feature-local` route or a separately approved isolated investigation, and must contain no production data, credentials, Secrets, OAuth/Aily apps, callbacks, or integrations. Local business UAT proves only a candidate and never replaces formal IDC acceptance; ngrok still requires separate approval.

Before commit, `scripts/fast-check.sh` can provide backend, frontend, or documentation feedback relative to an explicit base; shared and unknown paths fail safely to full scope. `.github/workflows/quality-gate.yml` still runs the complete backend regression against an isolated test database, frontend contract tests and production build, deployment-file checks, and bilingual documentation guard. After it passes, `push-images.sh` builds Git-SHA-derived immutable linux/amd64 images from a clean commit and pushes them to Harbor. It defaults to full scope, while a reviewed release may publish only backend or frontend; PostgreSQL mirroring is explicit. Component `k8s-deploy.sh` requires database skipping and preserves shared resources and the unselected Deployment. Full and component releases strictly verify rollouts, actual image identities, internal/external health paths, and MCP `initialize`. Real Aily, callback, identity, authorization, and business-flow acceptance runs only in IDC.

Before modification, every coding task uses the repository-level `.agents/skills/itom-task-routing` skill to confirm `production-fix|feature-local|code-candidate`, reads the matching route skill, and creates lifecycle evidence through `scripts/task-lifecycle.py --track`. Production fixes start from real IDC facts. New features may form a candidate in an isolated local Docker environment containing no production data, credentials, Secrets, OAuth/Aily applications, or callbacks; after `local_candidate_ready`, the containers, database, volumes, ports, and acceptance data remain available until the user explicitly completes UAT or requests shutdown. Code candidates must not start, stop, or alter an application runtime. Formal documentation and commit follow focused target acceptance, and a frozen candidate runs complete CI once. Every IDC write separately presents the exact commit, immutable tag, affected objects, data impact, disruption, rollback, and acceptance plan after CI, then records explicit user `approve-idc` authorization. A local candidate, user-passed local UAT, and CI are not production-release authorization.
...
Extend `notification_outbox` with recipient, idempotency key, retry count, next retry, provider message ID, redacted error, and send timestamps. User-scoped Aily rows may carry the internal `auth_user_id` in JSON payload while the recipient is unresolved; this is an additive payload contract and requires no schema migration.
...
Notification identity repair on 2026-08-06: investigation of `TK-202608-0036` found that the in-app notification was written, but the previous generic outlet silently skipped the Feishu outbox because 熊军 had only the login application's `AuthUser.external_id` and no verified identity for the Aily bot application. The outlet now enqueues enabled integrations by ITOM account even when the Aily identity is not mapped, stores `AILY_IDENTITY_NOT_MAPPED` as a redacted pending diagnostic, and re-resolves the active identity immediately before delivery. A Feishu OAuth `tenant_key + union_id` can automatically establish the bot-app mapping when the tenant is allowlisted and the configured bot application is under the same developer scope; open IDs from different applications are never mixed. Disabled integrations do not create backlog, identity-waiting rows do not consume retry attempts, and explicit disabled or conflicting mappings are not overwritten. Because `TK-202608-0036` predated this fix and had no Feishu outbox row, it is not replayed automatically; after deployment, 熊军 must sign in through Feishu once to establish the mapping, then a new controlled assignment notification is required for IDC acceptance.

Outbound-notification identity auto-mapping correction on 2026-08-11: live IDC evidence showed 28 active Feishu-login accounts but only 熊军 had an outbound mapping. Assignment rows for 胡书豪 and others were correctly queued yet remained `pending / AILY_IDENTITY_NOT_MAPPED`; those users had signed in through Feishu the same day. The previous implementation therefore incorrectly compared OAuth `tenant_key` with the Aily MCP JWT `tenant_id` allowlist and silently skipped mapping. The final contract separates the trust boundaries: the MCP tenant allowlist authorizes inbound tool calls only. After configured Feishu OAuth is verified and resolves to an active ITOM account, ITOM automatically maps the current `bot_app_id` with `tenant_key + user_id`, falling back to `tenant_key + union_id`. Outbound resolution prefers that bot-app mapping, while legacy manual/Aily mappings remain under the original tenant allowlist. Cross-app `open_id`, disabled mappings, account conflicts, and unknown identities remain denied. Existing users require no manual tenant or identity enrollment; their next normal Feishu sign-in after deployment creates the mapping and the background worker can then deliver existing identity-waiting outbox rows.

Bulk outbound-identity reconciliation on 2026-08-11: a read-only live probe proved that the existing directory `find_by_department` response already includes `open_id`, `user_id`, and `union_id`, while neither the login app nor the bot app currently has `tenant:tenant:readonly`; historical backfill therefore does not depend on the higher-privilege tenant-information API. The least-privilege path uses the first post-fix normal OAuth mapping, with its audit event, as the current bot app's unique trusted `tenant_key` anchor. When the worker encounters a waiting row for another active account linked to an active Feishu-synchronized employee, it uses that person's login-app `open_id` only to query one directory user, persists the returned tenant-level `user_id` (or `union_id` fallback) for the bot app, and then delivers the row. Other employees do not sign in again and administrators do not bind them one by one. Missing or ambiguous anchors, offboarded/unlinked people, explicit disables, conflicts, and lookup failures remain pending; lookup failure consumes no message-send attempt, and the login-app `open_id` is never sent to the bot API.

Feishu OAuth sign-in regression fix on 2026-08-12: live IDC logs proved that authorization URL generation, the app-token request, OIDC token exchange, and user-info lookup all succeeded. ITOM failed while committing the verified sign-in and its notification-identity audit in one transaction. The original action exceeded `audit_log.action VARCHAR(32)`, so PostgreSQL rejected the insert and rolled back the complete login transaction; SQLite regression tests had not enforced that length. OAuth mapping and organization-assisted reconciliation now use the bounded action codes `auto_map_aily_identity` and `auto_map_aily_identity_org`; trusted-anchor queries reuse the same constants, and a model-aware boundary test checks the persisted column length. This fix adds no migration, changes no identity trust boundary, and leaves existing accounts, mappings, notification outbox rows, and business data untouched.

Trusted public IDC TLS on 2026-08-10: the public-CA `*.snnc.cc` full certificate chain and matching private key were created as `itom/itom-snnc-cc-tls` (`kubernetes.io/tls`), and the Ingress now binds an exact TLS host for `itom.snnc.cc`. FortiGate retains TLS passthrough `183.60.58.58:30443 → 10.60.65.220:443`; DNS and `public_base_url=https://itom.snnc.cc:30443` were unchanged. Public `curl`, `openssl s_client -verify_return_error -verify_hostname`, and an existing browser login session all passed without bypassing certificate validation; the server presents a three-certificate DigiCert chain whose SAN covers `*.snnc.cc`. External `/mcp/` `initialize` returned the protocol response and `serverInfo`. Feishu Open Platform displays the same HTTPS callback URL, the new `card.action.trigger` subscription, and a published app; resaving the unchanged URL produced no error but did not emit a new challenge access-log line. To avoid changing production business tickets, this pass did not create a new ticket or click a signed card. A current-IDC real-role signed-card click remains a separate required business UAT and cannot be replaced by historical card evidence.

IDC frontend scheduling closure on 2026-08-10: all three nodes are Ready, schedulable, and free of pressure or taints; node02 is a normal production candidate and is no longer excluded based on its historical build-executor role. The target `itom-frontend` template explicitly declares an empty `nodeName` to clear the node01 pin left by manual recovery, then uses nodes 01/02 and hard hostname anti-affinity to span its two replicas. The backend remains an unpinned single-replica `Recreate` Deployment; the database StatefulSet, PVCs, Secrets, and business data are outside this application release. This policy does not change business, identity, data, or TLS contracts.
...
- trusted TLS now protects the IDC public entry; continue with security, performance, recovery, and current-version real-role UAT;
- open a user-approved PR from `feature/AI-agent-version` to `main`.

### Portfolio-governance boundary (2026-08-18)

This release adds portfolio governance only to the ITOM web UI and domain APIs, while project, WBS, risk, cost, permission, and audit services remain authoritative. It changes no Aily JWT, MCP transport, identity mapping, callback, notification, or Web Agent capability registration, and adds no portfolio read/write tool. `PortfolioGovernance` only adds allowlisted `page=portfolio_detail` plus the portfolio GLID to the existing web page context; it never reads DOM and grants no action. Any future Aily/MCP or Web Agent portfolio operation requires a separate capability risk decision, domain-service wrapper, record-level reauthorization, L3 preview/confirmation, idempotency, and real-role acceptance. The current web APIs are not implicit tool authority.

## 11. Definition of done

Each change includes implementation, tests, authoritative Chinese docs, and matching `docs/en` mirrors. The feature branch must first pass GitHub Actions for relevant unit, API/MCP integration, identity/permission, idempotency/retry tests, the production frontend build, and repository-contract checks. Automation never targets the IDC business database. Only a confirmed `feature-local` route may start the repository-defined isolated local application and test database; `production-fix` and `code-candidate` do not by default. Releases come from clean commits, use Git-SHA-derived immutable linux/amd64 images, receive separate exact release approval, and complete rollout, image, health-path, MCP, and real-role acceptance in IDC.
