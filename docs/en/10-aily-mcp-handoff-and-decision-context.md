...
### 3.5 ITOM web agent (design approved; WA0 Tasks 1–3 implemented)

The web agent uses two entrances and one capability core: web uses the current ITOM login, Aily retains its JWT/MCP identity, and both reuse only lower-level domain services and business guards. WA0 Tasks 1–3 implement disabled-by-default persistence and `admin_ai` permission foundations, fixed capability registration/request-time policy/recursive redaction, and the secure OpenAI-compatible model gateway. Every provider request binds public DNS validation to the literal IPs actually dialed while retaining the original Host, TLS SNI, and certificate verification name; environment proxies and redirects are disabled and stream termination is strict. Authenticated basic, streaming, forced-tool, and strict-JSON-Schema capabilities are probed independently and exactly; L2/L3 require all three feature flags. Purpose is server-owned, while primary/fallback call audits use independent transactions: `done` is withheld until success audit commit, and audit failure cannot commit caller work, mask cancellation, or corrupt safe fallback. Admin API/UI, web conversations, action orchestration, and business handlers remain pending. A normal business user's web agent remains limited to own service requests; BDO adds own IT requirements; IT staff receive system-wide guidance and progressively enabled writes under actual permissions and process assignment. Models, prompts, and administrator profiles are never authorization sources. The existing Record Creation Guide remains a deterministic fallback. See [`docs/en/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md).
