...
Every corrective code task is also governed by `skills/itom-fix-delivery/SKILL.md` and `scripts/task-lifecycle.py`: record grade, scope, and acceptance matrix at the start; pass focused real-target acceptance before assessing and synchronizing formal documentation, committing, or releasing; stop patch-style retries and complete root-cause review after the second failure of the same target; and run complete CI and IDC release only once for a frozen candidate. Git-ignored `.itom-task/` evidence contains no business data or secrets and separates development, pipeline-wait, and external-blocker time for weekly metrics.
