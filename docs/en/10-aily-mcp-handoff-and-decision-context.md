...
Task 6 establishes the generic L3 action boundary: preview trusts only a server-registered capability, its Pydantic input, and its fixed handler, then persists a stable digest only after recursive redaction. Idempotency is scoped by account + capability + key: an equal-digest retry returns the original action, while a different digest conflicts. Confirmation credentials store only a SHA-256 hash, expire after ten minutes, and are one-use. Confirmation first `FOR UPDATE` locks the action, then reloads the current account/roles/permissions/published profile and calls handler record-level reauthorization. A successful domain change, `AiAction` result, and generic audit commit atomically; a business failure first rolls back and then records a bounded redacted `failed` result separately, while handler-directed `commit/rollback` fails closed. Cancellation, confirmation, and replay are all constrained by ownership and terminal state. SQLite covers deterministic lock-call ordering and concurrency invariants; genuine PostgreSQL two-session proof that one action executes only once remains Task 9 IDC acceptance.
