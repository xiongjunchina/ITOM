...
**Current state: the P2 text-interaction loop has completed code, automation, the production frontend build, local Docker/ngrok, real-Aily multi-role conversation, normal-user identity, live bot receipt, and a normal-user same-ticket end-to-end run. The P2.1 server-side interactive-card contract is implemented. The card-action Skill is live in version `1.0.2`, its real `skill_*` is configured in local ITOM, and a real close/reopen card for the new UAT ticket has been sent. Actual user clicks for reopen, close, and rating remain.**
...
- with `card_action_skill_id`, resolution/reminder notifications become close/reopen cards and closure becomes a 1–5-star card; without it, delivery safely falls back to text. Buttons invoke an Aily Skill only, while all business mutations still go through MCP;
...
P2.1 interactive cards on 2026-07-30: the server now supports reliable card outbox delivery, close/reopen buttons on resolution/deadline reminders, 1–5-star buttons after closure, text fallback, `skill_*` validation, and idempotent existing-database migration. Cards follow the official `trigger_skill` contract and carry public ticket code, action, score, and idempotency key—never internal IDs or identity secrets. Focused automation reports `12 passed`, the full backend suite reports `271 passed`, the TypeScript/Vite production build succeeds, local PostgreSQL contains the new column, and port-8180 `/api/health` is healthy. The new custom agent generated and enabled `itom-service-request-closure` from a package with root-level `SKILL.md`; its page exposes a real `skill_*` suitable for the card action, and local ITOM now has that identifier configured with interactive-card readiness satisfied. An earlier anomalous “Create via Chat” result was neither saved nor published. The publish dialog reports online version `1.0.2` at 2026-07-30 09:06 with “no changes.” Real Aily then created and idempotently reused `TK-202607-0004`; after ITOM advanced it to `resolved`, an interactive card titled “Service request pending your confirmation” was delivered with outbox status `sent`, close/reopen buttons, and actions targeting the configured card-action Skill. The `xiongjun` user still needs to click the buttons so requester identity, reopen, close, and rating can be proven; the button loop is therefore not yet claimed as passed.
