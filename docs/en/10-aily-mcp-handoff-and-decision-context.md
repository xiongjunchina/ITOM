...
Software Release Center B1 on 2026-08-27 adds a Git-controlled read-only SemVer manifest, public release summary, About page, and OCI version labels. Future IDC release probes must also verify the health-endpoint product version. It changes no MCP `serverInfo`, tool, JWT/Origin/tenant/Agent validation, identity mapping, domain service, RBAC, or audit boundary and adds no database migration. Public release APIs expose no Git SHA, image, or database information.
