...
The web agent uses two entrances and one capability core: web uses the current ITOM login, Aily retains its JWT/MCP identity, and both reuse only lower-level domain services and business guards. WA0 Tasks 1–4 implement disabled-by-default persistence and `admin_ai` permission foundations, fixed capability registration/request-time policy/recursive redaction, the secure OpenAI-compatible model gateway, and `admin_ai`-only administration APIs for providers and four fixed profiles. Every provider request binds public DNS validation to the literal IPs actually dialed while retaining the original Host, TLS SNI, and certificate verification name; environment proxies and redirects are disabled and stream termination is strict. Authenticated basic, streaming, forced-tool, and strict-JSON-Schema capabilities are probed independently and exactly; probe state is persisted atomically and failure cannot leave a provider enabled or healthy. Provider governance and active references applied by profile publication are serialized across pods by a PostgreSQL transaction advisory lock plus deterministic provider-row locks. Secrets are encrypted write-only; profiles can only narrow code-registered capabilities, all publication-controlled settings remain in a version draft until successful publication atomically applies active settings and creates an immutable increasing version, and rollback copies and reapplies historical settings. Health and action-audit reads are aggregate/redacted allowlists, while usage is SQL-aggregated over a controlled 1–90-day window. Purpose is server-owned, while primary/fallback call audits use independent transactions: `done` is withheld until success audit commit, and audit failure cannot commit caller work, mask cancellation, or corrupt safe fallback. The administration UI, web conversations, action orchestration, and business handlers remain pending. A normal business user's web agent remains limited to own service requests; BDO adds own IT requirements; IT staff receive system-wide guidance and progressively enabled writes under actual permissions and process assignment. Models, prompts, and administrator profiles are never authorization sources. The existing Record Creation Guide remains a deterministic fallback. See [`docs/en/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md).
