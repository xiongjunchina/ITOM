...
### 3.5 ITOM web agent (design approved; WA0 Tasks 1–5 implemented)

Task 5 runtime accepts only a provably complete schema-marked version snapshot, bilingual prompts, valid capability/risk limits, a healthy compatible provider, and agreement with the newest active profile; unpublished, disabled, deleted, or inconsistent profiles fail closed. Ordinary-message retention comes only from the version captured at creation, never the live profile or `expires_at`: a captured zero remains nonpersistent after a later positive publication, while a captured 1–90-day decision and creation-time `expires_at` remain immutable, although new bodies stop when the current profile is withdrawn or audience-incompatible. Every body is recursively redacted first. `fallback_available` reflects only safe availability of the authenticated permission-aware Record Creation Guide, not WA1; pages are limited to 1–10,000.

The web agent uses two entrances and one capability core: web uses the current ITOM login, Aily retains its JWT/MCP identity, and both reuse only lower-level domain services and business guards. WA0 Tasks 1–5 implement disabled-by-default persistence and `admin_ai` permission foundations, fixed capability registration/request-time policy/recursive redaction, the secure OpenAI-compatible model gateway, `admin_ai`-only administration APIs for providers and four fixed profiles, and bootstrap/create/list/get/archive conversation APIs for the current login account. Conversation reads/archive are filtered by database `auth_user_id`, with the same 404 for every unowned ID; browser context accepts only a normalized local route, bounded identifiers, and at most 20 GLIDs, rejecting client roles/permissions, DOM/HTML, prompts, cookies/headers, external/protocol-relative/traversal-like paths, and extra fields. Unpublished/disabled profiles fail closed; retention 0 persists no ordinary message body, 1–90 sets `expires_at` and redacts messages first, and archive never deletes `ai_action` or security/business audit. Every provider request binds public DNS validation to the literal IPs actually dialed while retaining the original Host, TLS SNI, and certificate verification name; environment proxies and redirects are disabled and stream termination is strict. Authenticated basic, streaming, forced-tool, and strict-JSON-Schema capabilities are probed independently and exactly. Administration probes hold PostgreSQL advisory/row locks only in two short transactions around the network await, use `config_revision` to discard results after change/deletion, and cannot leave a failed provider enabled or healthy. Other provider governance and active references applied by profile publication remain serialized across pods by the same advisory lock plus deterministic provider-row locks. Secrets are encrypted write-only and are not plaintext-hashed. Profiles can only narrow code-registered capabilities; all publication-controlled settings remain in a complete schema-marked version draft until successful publication atomically applies active settings and creates an immutable increasing version. Rollback copies only a provably complete historical snapshot; legacy `{}`/incomplete versions fail closed and are never filled from current active settings. Health and action-audit reads are aggregate/redacted allowlists, while usage is SQL-aggregated over a controlled 1–90-day window. Purpose is server-owned, while primary/fallback call audits use independent transactions: `done` is withheld until success audit commit, and audit failure cannot commit caller work, mask cancellation, or corrupt safe fallback. The administration UI, message SSE/tool loop, action orchestration, and business handlers remain pending. A normal business user's web agent remains limited to own service requests; BDO adds own IT requirements; IT staff receive system-wide guidance and progressively enabled writes under actual permissions and process assignment. Models, prompts, and administrator profiles are never authorization sources. The existing Record Creation Guide remains a deterministic fallback. See [`docs/en/superpowers/specs/2026-08-01-itom-web-agent-design.md`](superpowers/specs/2026-08-01-itom-web-agent-design.md).
