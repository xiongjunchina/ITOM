...
**Dashboard** shows permitted KPIs, pending work, recent activity, and project/requirement/team summaries. ITSM is split into four independent cards: service requests, changes, incidents, and problems. Each card shows that module's open/in-progress count and P1–P4 priority distribution, plus its own SLA, approval, implementation, known-error, or closure metrics; accounts with task view permission also see open Bug, Bug-fix-child-task, delegated-task, and requirement-development counts. Cross-module ITSM totals are no longer duplicated. Administrators can configure role landing pages. The bell shows in-app notifications; opening a notification does not replace the required action on the source record. Use **Mark all as read** to write read receipts for the current account's unread notifications, or **Clear read** to soft-delete notifications already read by that account; neither action changes the source record.

## 3. ITSM

### Quick routing for IT staff

Business users normally do not need to open these pages: describe the request to Feishu Aily. A normal business user can create only a service request, while a department-appointed BDO can register IT requirements. Signed-in IT team members and system administrators can use the header's **Record Creation Guide** for two to four lightweight temporary questions. Answers are not saved, the recommendation is overrideable, and the UI shows only entries the current account can actually create. The in-app **User Manual** entry is temporarily visible to system administrators only, pending a unified rewrite after the main features are complete. In **Learning & Growth → Training Development**, participants are shown by department: checking a department selects all of its IT-team members, after which individual selections can still be adjusted. Saving a full-department selection freezes that attendee scope; the list shows the department name (plus any individual attendees outside it), and later transfers do not alter historical attendees or point recipients. The list, detail, and creation forms for all six record types include a one-line use case and **View examples**. Choose a service request for an individual user's fault, assistance, or request; an incident for a broad interruption; a problem for recurrence requiring root-cause analysis; a change for planned production work with risk, window, or rollback requirements; an IT requirement for a new system, function, optimization, or refactor; and a project only for delivery that meets project-management conditions. The detail-page **Related records** section shows existing relationships bidirectionally only when the current account can see both records; invisible records are not shown. Authorized IT staff can click **Upgrade to incident / Create root-cause problem / Create remediation change / Create linked project** from service-request, incident, problem, or requirement details. The system safely pre-fills source context, requires target fields plus a link reason of at least five characters, then creates the independent target workflow while retaining the source. A network retry does not create a duplicate target; the same operation key with different content is rejected. It does not change Aily permissions, fields, confirmation, reopen, or rating.
...
CMDB maintains configuration items and relationships. The Bug form reads its “Affected system” options from current non-retired CMDB items and does not maintain a second system dictionary. Every CI has a technical owner for technical operation and maintenance; only an Application CI additionally has a product manager for Bug confirmation and verification. They may be the same person, but are not duplicate roles. In the CMDB create/edit form, administrators select an IT-team product manager for Application systems; if Bug registration reports a missing product manager, add it there and retry. A later change affects only new Bugs because each Bug keeps its registration-time snapshot. SLA defines response and resolution targets and feeds service metrics. Incidents record restoration work; changes record risk, approval, implementation, and rollback; problems record root-cause analysis and permanent fixes. Vendors and contracts maintain supplier governance and renewal data. Knowledge articles are created, published, linked, and archived. These pages share search, filters, sorting, pagination, and permission-controlled editing.
...
Business users register requirements. IT teams clarify, score, evaluate solutions, and route them to development, project delivery, deferment, or rejection. Task Tracking shows the resulting tasks, owners, dates, and completion. While a requirement is in Implementation, every built-in IT role can repeatedly register and edit task rows from the requirement detail or Task Tracking page and maintain the task description, assignee, planned date, and planned/actual effort. A handler without maintenance rights can update only the status and actual effort of their own task. An IT user with development-task maintenance may delete a task that is not In Progress; only the system administrator may delete an In Progress task. Buttons reflect current capability, but the server rechecks authorization, so existing task and requirement data is not overwritten by the UI change. Scoring Rules defines dimensions, weights, and score bands. The process and its assignees/CC recipients are controlled by workflow configuration.

### 5.3 Task Management

Open **Task Management** from the left navigation. Development Tasks has “Requirement Development”, “Bug Fix”, and “Project Development”; Delegated Tasks is a separate page. Historical Task Tracking bookmarks still redirect to Requirement Development.

Requirement Development may be registered with no requirement and linked later; the template's requirement-code column is optional. A supplied code must match a real implementing requirement, while task name and active IT assignee stay required. Import validates rows independently and only appends. Project Development requires a project and makes WBS optional; assignees append progress and registrars see the full history. Bug Fix follows the existing approval loop; its code opens detail, the affected system can change only in the workflow correction window, the creator may delete only before the next node is viewed/handled, and administrators retain deletion authority.

Delegated Tasks record work from tickets, problems, incidents, technical research, and other non-project work. **Update Progress** appends author, time, status snapshot, completion percentage, and comment without overwriting history. Assignment notifies the assignee; assignee progress/status/effort/completion updates notify the registrar through in-app and configured Feishu delivery. The registrar may delete an unassigned registered task; once assigned, only an administrator may delete it before closure.
...
Shows active headcount, training, campaigns, hiring demand, workload, open task counts, and points. The dashboard layout is personal to the current account.
...
Headcount contains Position Definitions and Hiring Needs. Administrators and CIO can edit rows inline or in detail, delete, export, download templates, and batch import. Learning & Growth contains Training Development and Learning Tasks. Training participants are grouped in collapsible department directories and selected through person checkboxes. A full-department selection is displayed as a department name rather than an individual-name list, and the attendee scope is frozen when recorded for point calculation. Administrators, CIO, or the registrar may edit or delete a training activity; changing the host or participants in the current unpublished and unlocked period retracts old points and re-awards them under the latest rule. Historical, published, and locked periods cannot be deleted or changed in a way that changes point recipients, while non-point metadata remains editable. Learning Tasks record a cycle goal, progress, evidence, and notes; 0–100% progress converts to team-contribution points. Activity Points maintains campaign and contribution records; its sibling Point Rules tab owns team-contribution event configuration. Current-period automatic activity points refresh every 30 seconds and can be refreshed manually; rule changes or deactivation update the activity views, while award-time entries and published/locked performance stay unchanged. Team Charter maintains vision, annual goals, and working principles.

## 7. Process center

Process Definitions configure versions, processing/approval node types, handlers, CC recipients, roles, and automation levels. The list follows the left-navigation order: ITSM (Service Request), ITSM (Change), ITSM (Incident), ITSM (Problem), Project, Requirement, and Bug Management. New records use the published version; existing records retain their creation snapshot. For a process already referenced by records, CC recipients remain editable and take effect only when a node activates after the save; node order, node type, handler, autonomy level, and SLA stay locked and require Save as New Version. Existing tasks display their creation-time CC snapshot, and sent notifications are not replayed or rewritten. In Bug Management, the Development Fix node is executed through the developer/tester assignees on the child repair tasks rather than the development leader. Process Monitor filters running records by type, state, current node, owner, and time. Approval nodes support Approve or Reject; rejection requires a reason, while approval comments are optional. Processing nodes use Complete Step. CC recipients receive notifications without a task. Current-period Activity Points leaderboard totals resolve automatic `team_contribution` events against the effective Point Rule; a disabled rule displays zero, while campaign and manual entries retain their ledger amount. Historical periods and published or locked performance remain frozen. `role_result` rows are excluded, and the result is not the normalized Performance score.

## 8. System administration

- **Organization**: the page initially expands only the company and first-level department names; click a department name or expand arrow to recursively reveal child departments and people. The complete department/person tree remains available to selectors, synchronization, and authorization. Maintain the people master data and define the digital-team scope as the union of selected department members and individually selected people; create business domains from organization departments and optionally include descendants. In a mixed Test/vendor organization, select only the contractors who participate in IT work and performance reviews—their colleagues are not included.
- **Users & Groups**: manage accounts, linked people, roles, groups, status, provisioning, and initial passwords. Linked Person uses the complete company organization, shows a readable person and department, and is not limited to the digital-team scope. A synced business person outside the IT scope can be automatically provisioned on a safe first Feishu login with the requester role and an email-prefix username; IT-scope people remain in administrator approval. Administrators can rebind or clear the person; clearing does not delete the person, department placement, group membership, or history. Groups represent professional resource pools and can grant roles.
