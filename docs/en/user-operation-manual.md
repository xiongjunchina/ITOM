...
CMDB maintains configuration items and relationships. The Bug form reads its “Affected system” options from current non-retired CMDB items and does not maintain a second system dictionary; administrators should maintain the CMDB item and product-manager data. SLA defines response and resolution targets and feeds service metrics. Incidents record restoration work; changes record risk, approval, implementation, and rollback; problems record root-cause analysis and permanent fixes. Vendors and contracts maintain supplier governance and renewal data. Knowledge articles are created, published, linked, and archived. These pages share search, filters, sorting, pagination, and permission-controlled editing.
