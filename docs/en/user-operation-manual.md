...
The Development Tasks page has “Requirement Development” and “Bug Fix” tabs. Requirement Development shows the existing requirement-decomposition tasks. Bug Fix follows registration, product-manager confirmation, development-leader creation of multiple fix/test tasks, developer execution, and product-manager verification/closure. The Bug's affected-system options come from active CMDB items. Any IT team member may register a Bug; only the affected-system product manager confirms and verifies it, while developers maintain execution fields for tasks assigned to them.
