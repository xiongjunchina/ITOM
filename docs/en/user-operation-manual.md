...
The Development Tasks page has “Requirement Development” and “Bug Fix” tabs. Requirement Development shows the existing requirement-decomposition tasks and offers **Download Template** plus **Bulk Import** for `.xlsx` files up to 5MB. Each row must contain an implementing requirement's real code, a task name, and an active IT assignee; the system validates dates, effort, status, and the relationship row by row. The result reports created rows and failed row numbers/reasons. Failed rows do not block other valid rows, and import adds only new tasks without updating or overwriting existing tasks. Bug Fix follows registration, product-manager confirmation, development-leader creation of multiple fix/test tasks, developer execution, and product-manager verification/closure. The Bug's affected-system options come from active CMDB items. Any IT team member may register a Bug; only the affected-system product manager confirms and verifies it, while developers maintain execution fields for tasks assigned to them.
