...
### 11.2a IT-staff web routing and transfer permission (phases A/B implemented; create-and-relate pending)
...
Phase-B relation reads are now trimmed by visibility of both source and target, and detail pages display only that safe result, so a related record cannot leak access to another record. For a phase-C create-and-relate action, an actor needs both source-record read scope and the target record's `create` permission. The target domain service continues to validate required fields, workflow node, approval, role, and data scope. System-level administrator authority does not replace a business submitter's service-request confirmation and does not change the Aily normal-user boundary.
