...
2. **Synced records are locked locally**: a person's HR base fields (name / employee number / gender / date of birth / employment type / supervisor / office location / department / contact / hire date / status) and department structure fields are rejected for edits by the backend (SYNCED_READONLY); **locally, only the IT extensions are maintained** — a person's position/skills/remarks, and a department's it/business/audit classification. Feishu manages HR; New_AOM manages the IT extensions.
