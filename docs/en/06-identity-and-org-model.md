...
A model profile controls wording, knowledge scope, disabled capabilities, and maximum risk only; it never authorizes. Task 6 L3 preview accepts only the current login and a server-registered capability, revalidates with the registered input model, and recursively redacts it; model- or client-claimed capability, risk, role, handler, SLA, queue, workflow, assignment, and state never authorize. A confirmation token binds the ITOM user, owned web conversation, capability, payload digest, and ten-minute lifetime, stores only its hash, and is consumable once. After locking the action row, confirmation reloads the active account, current effective roles/permissions/published profile, then requires the fixed handler to re-authorize data scope, record state, ownership, current workflow assignment, and domain invariants. Auditors cannot execute L3, unowned actions return the same 404 as unowned conversations, and cancellation is owner-only and `prepared`-only. Genuine PostgreSQL two-session contention on one action remains Task 9 IDC acceptance.
