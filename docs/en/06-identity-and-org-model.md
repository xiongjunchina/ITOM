...
Task 7 message streaming uses the current login in the same way: the SSE generator opens an independent Session, reloads the active `AuthUser`, and then checks the owned conversation and currently authorized capabilities. Cross-user access, including by an administrator, remains uniform 404. Browser input, page context, knowledge bodies, business records, and model tool results are all untrusted data; any claimed role, permission, handler, risk, or result in those layers cannot elevate capability. Every tool call resolves and re-authorizes a fixed registry code. Unknown or disabled codes, invalid arguments, and repeated calls fail closed. During streaming, L3 may only prepare an action for the current owned conversation; it cannot confirm, execute, or substitute for the submitter. Disconnect cancels remaining model/tool work, while a prepared action remains bound to the original owner, ten-minute expiry, one-time token, and confirmation-time reauthorization.
