...
The final requester-confirmation task is submitter-only; even administrators cannot confirm for the submitter. Web and MCP use the same domain semantics: confirmation closes, while rejection with a reason rewinds to the nearest real handling step. Proactive messages select the account's most recently used active Feishu identity and include user-visible progress only, never internal notes, root cause, or approval data. An interactive-card button only invokes an Aily Skill and is not server authorization. Forwarded callbacks are disabled; the clicking user must still map through Aily JWT to an active ITOM account, after which MCP verifies that the user is the request submitter. Card `skill_input` never carries Feishu identifiers, JWTs, ITOM internal primary keys, or internal handling fields.
