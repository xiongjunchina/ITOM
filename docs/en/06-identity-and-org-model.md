...
- The Organization UI intentionally shows only the company and first-level department names for readability. The backend retains the complete department/person tree for selectors, synchronization, data scope, and authorization; no existing organization data is deleted or truncated.
