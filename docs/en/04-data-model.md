...
### 1.16 Software release manifest (B1; non-database model)

- `release/current.json` stores only the schema version and current immutable-manifest filename. The filename must be a safe basename and cannot traverse paths.
- `release/releases/v<SemVer>.json` stores bilingual product identity, version, channel/status/date, rollback compatibility, and bilingual highlights. Git audits manifests; released versions are not overwritten.
- Runtime performs strict Pydantic validation, while CI checks SemVer, filename, current pointer, frontend package version, and bilingual release-note mirrors.
- `ui_branding_version.config.legal` adds developer, vendor, copyright, website, support, license, and third-party-notice fields inside the existing JSON configuration, requiring no column or migration. Branding revisions and software versions remain separate.
- B1 adds no release, deployment, or acknowledgement table and changes no `auth_user`, organization, RBAC, Aily/MCP, or business-record model.
