...
- [ ] Confirm under `SELECT FOR UPDATE`, lock/refresh the owned active conversation before runtime governance, pass only `ActionUnitOfWork.lock_one()/update_locked()`, immutable `FrozenActionRecord` snapshots, and an opaque `LockedActionRecord` into record authorization/mutation, and atomically persist business result/action/audit. Preview authorization/preview use only a preview-local actor context plus a `ReadOnlyActionData` facade with no Session-like attributes. The facade recursively allowlists the complete SQLAlchemy AST to one direct mapped table's explicit scalar columns, same-table safe predicates/order clauses, compile-time nonnegative bounded limit/offset, and max + 1 overflow detection; entity/relationship/eager projection, joins/aliases/subqueries/CTEs, aggregates/windows/functions, text/raw SQL, cross-table references, row locks at any depth, and dynamic/negative/excessive pagination fail closed. Any nonempty `_prefixes`, `_suffixes`, `_statement_hints`, or `_hints` must also fail before SQL execution without parsing or a string allowlist, and `ReadOnlyActionData` plus `lock_one()` share this gate. Mutation handles are privately bound to the exact issuing UoW, Session, outer transaction, and savepoint, consumed and rotated after a successful update, and rejected when forged, cross-scope, terminal, or reused. After exception rollback, store a separate redacted failure state.
