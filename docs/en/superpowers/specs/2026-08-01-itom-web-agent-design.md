...
Task 7 implements message POST-SSE with the fixed event set `meta|delta|message|action|error|done`. A normal completion has exactly one `done`, no success event follows `error`, every `data` field is one-line JSON, and responses are private/no-store and vary by Authorization. The router passes only an immutable account ID. A turn keeps scalar snapshots only; start, capability discovery, each tool, L3 preparation, and final persistence use separate short Sessions, and Gateway releases provider-selection transactions before network waits. Prompt construction separates platform security instruction, immutable published profile, per-turn authorized capability schemas, and explicitly untrusted knowledge/business/page-context/user bodies, with stable line/sentence/fragment fingerprints blocking partial leakage. Every tool call resolves a fixed registry code and re-authorizes; registered input models reject illegal arguments and model-supplied handler/risk/role/result. Repeated calls, more than four rounds, or event/token/text/tool/time budget exhaustion stop safely. L1/L2 receives only `ReadOnlyActionData` and an immutable actor; PostgreSQL is transaction-read-only with a statement timeout before reads, and Session/ORM/Core DML/raw SQL escape attempts fail closed. L3 calls server `prepare_action()` only and receives a server-rendered `server_preview/prepared_not_executed` result. Ordinary model prose can only be `advisory/not_executed`, never an ITOM success or status result. Provider outage, timeout, malformed/truncated protocol, and disconnect immediately stop waiting and event emission and use a permission-aware deterministic native fallback. The system does not claim to force-kill arbitrary synchronous Python workers: L1/L2 is bounded by read-only database access and timeout, while L3 may at most retain prepared until expiry and never executes. Idempotency uses a server-key HMAC over normalized raw input. Final completion uses a fresh short transaction to revalidate the active account, owned active conversation, exact current profile/version, and streaming placeholder. Only a redacted final body may become completed under captured retention; zero-day retention stores bodyless idempotency metadata only. Owner/idempotency/runtime failures after endpoint acceptance remain HTTP 200 SSE `error→done`; real PostgreSQL two-Session and real ASGI disconnect evidence remains Task 9.
...
- Prepare follows two ordered paths: an existing key is first probed without a lock, then replay/race recovery locks `AiAction` first and the owned active conversation second before digest comparison; a brand-new action locks/revalidates the conversation after preview, rechecks the same key without taking an action lock, and inserts only while it is still absent. Confirmation retains the action row lock, then locks and refreshes the owned conversation row, proves that the conversation-captured profile/version/provider remains runnable under the governance lock order, and gives record authorization/mutation only an immutable actor context plus `ActionUnitOfWork.lock_one()/update_locked()`, immutable `FrozenActionRecord` snapshots, and an opaque `LockedActionRecord`. Module-private state binds each locked handle to the exact issuing UoW, Session, outer transaction, and savepoint; a successful update consumes it and returns a replacement handle whose snapshot merges prior/current changes, while forged, cross-UoW/session/transaction, terminal-transaction, and repeated handles fail closed. Domain mutation, success result, and audit stay in a nested savepoint; failure rolls back only that savepoint and lets the same outer transaction commit the bounded terminal fact. Conversation archive follows the same row-lock-and-refresh discipline before commit. Handler code is trusted in-process extension code, not a hostile-code sandbox.
