...
- 质量门禁通过后，`deploy/k8s/push-images.sh` 只接受干净提交，使用已验证的 `mirror.gcr.io` 官方 Docker Library 缓存与固定摘要取得 Python/Node/Nginx/PostgreSQL 基础镜像，构建并校验 linux/amd64，以 `git-<commit前12位>-linux-amd64` 为默认不可变标签后推送 Harbor；镜像构建不启动本地 ITOM，也不依赖 Docker Hub 匿名限流。脚本显式读取当前 Docker CLI context 的 daemon 地址并传给 `skopeo`，兼容默认 Docker socket 及 OrbStack、Colima、Rancher Desktop 等非默认 socket。
