...
- 修复任务必须先按 `skills/itom-fix-delivery/SKILL.md` 用 `scripts/task-lifecycle.py` 建立本机生命周期记录。定向真实目标验收是正式文档和提交的前置门禁；同一验收目标第二次失败必须先完成根因复盘；冻结候选只允许一次完整 CI 和一次 IDC 发布尝试，失败后不得原样重跑。该记录不保存业务数据或密钥，不改变 API、领域模型或 IDC 数据，仅用于交付门禁、分段计时和周度指标。
