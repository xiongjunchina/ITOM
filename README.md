...
- **WBS 表格交互**：WBS 宽表支持 Excel 式冻结窗格（表头 + 左侧前三列），复用全局唯一底部悬浮横向滚动条及表内原生兜底，同时保留专用列宽与行高拖拽调整；“阶段”和“操作”列按最小可用宽度紧凑展示；完成度提供 0%/50%/100% 预设并支持自定义 0–100% 整数；父子任务进度自动级联/汇总；布局按项目保存在当前浏览器。表格底部可按 50（默认）/100/200/全部控制显示，并回显当前显示数与总数；有限行数按既有顺序截取并自动补齐祖先。清单支持复选框/全选和选中后出现的批量删除。填写不晚于今天且不早于实际开始的实际结束会自动完成任务，未来实际结束被拒绝；当前完成度为 100% 的任务锁定实际日期、禁选并锁定删除、增加子任务和结构调整。经授权用一次独立请求把完成度纠正到 100% 以下后，系统清空实际结束、保留首次完成时间并恢复日期及结构操作；同一请求不得同时重开并修改实际日期。结构调整仍受权限、引用关系以及已完成同级/后代保护约束，服务器重算顺序和 WBS 编码并保留审计。
