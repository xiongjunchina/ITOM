...
- **ITOM 网页智能体（设计已确认，待实施）**：采用网页登录与 Aily MCP 身份隔离、下层领域能力统一的方案。普通业务用户只处理本人服务请求，BDO 增加本人 IT 需求，IT 员工按真实角色/权限/数据范围/流程任务获得全模块指导和分阶段受控写操作。模型和提示词不授权，能力由服务端注册，L3 写操作必须预览、明确确认、幂等和审计；现有“创建单据指引”保留为模型故障降级。实施按 WA0 基础治理 → WA1 只读顾问 → WA2 业务闭环 → WA3 IT 员工写操作 → WA4 效果治理推进，详见 [`docs/superpowers/specs/2026-08-01-itom-web-agent-design.md`](docs/superpowers/specs/2026-08-01-itom-web-agent-design.md)。
...
M45–M46 是冻结版 Helpdesk 历史里程碑，仅保留在 `v1.0.0-feishu-helpdesk`。Aily-MCP 新开发线已完成 P0（移除 Helpdesk、协议/身份/消息）、P1（真实服务目录、动态表单、服务请求/BDO 需求登记、派单）和 P2（解决通知、确认/重开、评价闭环），P3 飞书审批按用户决定暂缓；IDC 发布加固继续执行。后续网页智能体独立使用 WA0–WA4 编号，避免与 Aily 阶段混淆。
...
- **ITOM Web Agent (design approved; implementation pending)**: web login and Aily MCP identity remain isolated while lower domain capabilities are shared. A normal business user handles own service requests only, BDO adds own IT requirements, and IT staff receive system-wide guidance and progressively controlled writes under actual roles, permission, data scope, and process assignment. Models/prompts never authorize; executable capabilities are server-registered; L3 writes require preview, explicit confirmation, idempotency, and audit; the current guide remains the model-outage fallback. Delivery proceeds through WA0 foundation → WA1 read-only advisor → WA2 business loop → WA3 IT-staff writes → WA4 governance. See the [design baseline](docs/en/superpowers/specs/2026-08-01-itom-web-agent-design.md).
