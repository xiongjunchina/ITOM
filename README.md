...
- **全局表格交互**：主页面统一业务清单的表头始终相对应用内容区吸顶，纵向滚动长清单时仍保持在页面内可见；所有超出内容区宽度的表格由 ITOM 为当前活动宽表提供一条浏览器原生、可拖拽的底部悬浮横向滚动条，即使尚未滚动到清单最后一行也能访问屏幕外字段。该滚动条与表格正文双向同步；只有它已完成尺寸测量且可用时才隐藏表内重复条，测量或浏览器异常时表内原生横向滚动条仍可操作。表头定位仍绑定 `.app-content` 真实滚动容器。统一业务清单的每个可见表头分隔线支持鼠标拖拽和键盘左右键微调；鼠标靠近分隔线时显示调整光标，无需常驻醒目手柄，未预设宽度的字段也有默认可调宽度且变化会实际应用到表格布局。布局按当前账号、路由及列集合隔离保存，同页多张表不会相互覆盖。WBS 额外支持前三列冻结，需求总览额外冻结编号与标题两列。
...
- **WBS 表格交互**：WBS 宽表支持 Excel 式冻结窗格（表头 + 左侧前三列）、与其他宽表相同的浏览器原生底部悬浮横向滚动条及表内原生兜底、列宽与行高拖拽调整；完成度提供 0%/50%/100% 预设并支持自定义 0–100% 整数；父子任务进度自动级联/汇总；布局按项目保存在当前浏览器。已确认完成的 WBS 任务锁定删除和结构调整；未启动任务/子树可调整父项或同级拖拽排序，服务器重算顺序和 WBS 编码并保留审计。
- **受控批量删除**：业务清单可勾选 1–100 条请求批量删除；服务器逐项重用单条删除的授权、状态、引用和审计约束，支持部分成功和逐条拒绝原因。覆盖工单、需求、Bug、三类任务、培训、CMDB 配置项/关系、服务目录/服务项；不提供级联绕过。
