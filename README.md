...
- **Task 7 受控流式编排**：`POST /api/assistant/conversations/{id}/messages` 只产生 `meta|delta|message|action|error|done`，每轮最多四次工具调用，并限制事件、Token、文本和总耗时。平台指令、已发布档案、授权能力 schema 与明确标注为不可信的页面/知识/业务/用户正文严格分层；工具按固定 code 重新授权，模型自报 handler/risk/role/result、未知能力和非法参数均失败关闭。L3 流内只调用 Task 6 `prepare_action()` 返回服务端预览和确认 Token，绝不确认或执行；错误、超时、非法协议及断流不保存完成回答，也不在错误后发送成功事件。消息幂等键支持安全重放和冲突拒绝，保留期 0 只保存无正文幂等元数据。管理 UI 和具体业务能力仍由 Task 8+ 实施。
...
- **Task 7 guarded streaming orchestration**: `POST /api/assistant/conversations/{id}/messages` emits only `meta|delta|message|action|error|done`, with four tool rounds and bounded events, tokens, text, and turn time. Platform instruction, published profile, authorized capability schemas, and explicitly untrusted page/knowledge/business/user bodies remain separate prompt layers. Every fixed tool code is re-authorized; model-supplied handler/risk/role/result, unknown capabilities, and invalid arguments fail closed. L3 streaming calls Task 6 `prepare_action()` only and returns a server preview/token without confirm or execution. Provider/protocol/timeout/disconnect failures persist no completed answer and never emit success after error. Client-message idempotency safely replays or rejects conflicts; retention zero stores bodyless idempotency metadata only. Administration UI and concrete business capabilities remain Task 8+ work.
