...
- **ITOM 网页智能体（设计已确认；WA0 Task 1–3 已实现）**：采用网页登录与 Aily MCP 身份隔离、下层领域能力统一的方案。当前已具备默认关闭的持久化与权限基础、固定能力注册/实时策略/递归脱敏，以及仅支持 OpenAI-compatible `/chat/completions` 的安全模型网关；每个真实出站请求先复验 HTTPS、部署白名单和全部公网 DNS 地址，再只向该次已验证的 IP 集合拨号，同时保留原主机用于 Host、TLS SNI 和证书校验，且拒绝重定向。探测分别验证认证基础响应、完整流终止、强制指定工具及严格 JSON Schema；L2/L3 仅使用四项探测成功且近期健康的模型。流在 `[DONE]` 立即关闭，协议异常失败关闭；purpose 只接受服务端枚举，主备尝试由独立事务写最小化脱敏审计，审计失败不提交调用方事务、不掩盖取消或破坏安全回退，成功审计落库前不发送 `done`。管理员配置/API、网页会话、动作编排、业务处理器和 UI 尚未实现。模型和提示词不授权，L3 写操作仍必须预览、明确确认、幂等和审计；现有“创建单据指引”保留为模型故障降级。实施继续按 WA0 基础治理 → WA1 只读顾问 → WA2 业务闭环 → WA3 IT 员工写操作 → WA4 效果治理推进，详见 [`docs/superpowers/specs/2026-08-01-itom-web-agent-design.md`](docs/superpowers/specs/2026-08-01-itom-web-agent-design.md)。
...
M45–M46 是冻结版 Helpdesk 历史里程碑，仅保留在 `v1.0.0-feishu-helpdesk`。Aily-MCP 新开发线已完成 P0（移除 Helpdesk、协议/身份/消息）、P1（真实服务目录、动态表单、服务请求/BDO 需求登记、派单）和 P2（解决通知、确认/重开、评价闭环），P3 飞书审批按用户决定暂缓；IDC 发布加固继续执行。后续网页智能体独立使用 WA0–WA4 编号，避免与 Aily 阶段混淆；WA0 Task 1–3 的持久化、策略/脱敏和安全模型网关已实现，Task 4+ 仍待实施。
...
- **ITOM Web Agent (design approved; WA0 Tasks 1–3 implemented)**: web login and Aily MCP identity remain isolated while lower domain capabilities are shared. The current foundation includes disabled-by-default persistence and permissions, fixed capability registration/live policy/recursive redaction, and a secure OpenAI-compatible `/chat/completions` gateway. Every real outbound request revalidates HTTPS, the deployment allowlist, and all public DNS answers, then dials only that request's validated IP set while preserving the original host for Host, TLS SNI, and certificate verification; redirects are refused. Probes independently prove authenticated basic response, valid stream termination, an exact forced tool call, and strict JSON Schema output; L2/L3 require all four checks on a recently healthy model. Streams close immediately on `[DONE]` and malformed protocol fails closed. Purpose is a server-owned enum, and primary/fallback attempts use independent minimal redacted audit transactions: an audit failure cannot commit caller work, mask cancellation, or corrupt safe fallback, while `done` is withheld until successful audit persistence. Admin configuration/API, web conversations, action orchestration, domain handlers, and UI remain pending. Models/prompts never authorize; L3 writes still require preview, explicit confirmation, idempotency, and audit; the current guide remains the model-outage fallback. Delivery continues through WA0 foundation → WA1 read-only advisor → WA2 business loop → WA3 IT-staff writes → WA4 governance. See the [design baseline](docs/en/superpowers/specs/2026-08-01-itom-web-agent-design.md).
