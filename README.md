...
- **Task 7 受控流式编排**：`POST /api/assistant/conversations/{id}/messages` 只产生 `meta|delta|message|action|error|done`，使用 `Cache-Control: no-store, private`、`Vary: Authorization`，每轮最多四次工具调用，并限制事件、Token、文本、工具和总耗时。平台指令、已发布档案、授权能力 schema 与明确标注为不可信的页面/知识/业务/用户正文严格分层，平台/档案指令还按“完整行至少 12 个规范化字符、长片段至少 24 个字符、两个 12–23 字符片段”的保守阈值检查部分泄漏。L1/L2 工具只接收 `ReadOnlyActionData` 和不可变 actor DTO，在独立回滚 Session 的 PostgreSQL 只读事务及 `statement_timeout` 下由受控 worker 运行；原始 Session/ORM/Connection/Engine、事务控制、DML 和 ORM mutation 均不可达。L3 流内只调用 Task 6 `prepare_action()`，服务端生成 `server_preview/prepared_not_executed` 消息，绝不确认或执行；其他模型正文一律标为 `advisory/not_executed`，不作为 ITOM 状态或操作结果。客户端断开会立即停止协程等待和事件发送；Python 线程不能被强杀，后台 L1/L2 仍受只读/数据库超时约束，已开始的 L3 prepare 最多形成到期前的 `prepared`，不会执行。原始正文和页面上下文只用于服务端密钥 HMAC 幂等摘要，持久化正文仍递归脱敏。终态写入在新短事务内重新锁定并验证活动账号、本人 active 会话、精确当前档案/版本和 `streaming` 占位消息；任何竞态失败关闭。管理 UI 和具体业务能力仍由 Task 8+ 实施。
...
- **Task 7 guarded streaming orchestration**: `POST /api/assistant/conversations/{id}/messages` emits only `meta|delta|message|action|error|done` with `Cache-Control: no-store, private` and `Vary: Authorization`, four tool rounds, and bounded event/token/text/tool/turn budgets. Platform instruction, published profile, authorized capability schemas, and explicitly untrusted page/knowledge/business/user bodies remain separate, with conservative partial-leak fingerprints (complete normalized lines at 12+ characters, strong fragments at 24+, or two distinct 12–23-character fragments). L1/L2 handlers receive only `ReadOnlyActionData` and an immutable actor DTO in a dedicated rollback-only PostgreSQL read-only transaction with `statement_timeout`; raw Session/ORM/Connection/Engine, transaction control, DML, and ORM mutation are unreachable. L3 streaming calls Task 6 `prepare_action()` only and returns a server-authored `server_preview/prepared_not_executed` message. All other model prose is `advisory/not_executed`, never an authoritative ITOM result. Disconnect stops coroutine waiting and event emission immediately; Python worker threads are not force-killed, so background L1/L2 remains read-only and DB-time-bounded, while an already-started L3 prepare may only remain `prepared` until expiry and never executes. Raw normalized content/page context feeds a server-key HMAC idempotency digest while durable bodies remain redacted. Final completion uses a fresh short transaction to lock and revalidate the active account, owned active conversation, exact current profile/version, and streaming placeholder. Administration UI and concrete business capabilities remain Task 8+ work.
