...
- **任务管理（M82）**：开发任务包含“需求开发 / Bug 修复 / 项目开发”，另设“委派任务”。需求开发允许先登记不关联需求、后续再补充关联，Excel 模板与导入同样把关联需求编号作为可选项；项目开发要求关联项目、WBS 可选，用于补充项目 WBS 尚未细拆到开发活动的工作。开发任务清单统一显示“登记人”：网页直接登记取当前登录人员，需求转化取原需求登记人，项目/WBS 转化取项目经理，Bug 修复取 Bug 登记人；历史或系统管理员无人员映射时允许显示为空且仍可从账号审计追溯。项目开发清单以“任务状态”为唯一状态口径，并把追加式进度中的最新有效百分比派生为“完成度”；文字批注只在详情的“进展说明”时间线展示。第一次有效进度会把待处理任务启动为进行中，普通进度最高 99%，只有显式“完成任务”并填写完成说明后才原子写入已完成、100%、完成时间和完成记录；旧进度不迁移、不覆盖。任务指派时通知负责人，负责人更新进度时通知登记人，统一通过站内通知及已启用的飞书 Outbox 出口发送。Bug 支持编辑所属系统、编号进入详情和与流程单据一致的创建人回改/删除窗口，管理员保留全程删除权。所有任务写入继续由服务端 RBAC、记录范围、状态、审计和软删除校验，不改写存量任务。
