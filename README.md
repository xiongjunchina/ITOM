...
- **Aily + MCP 正式基线**：P0 已完成 MCP 内嵌、首次注册协议发现、Aily JWT/Origin/租户/Agent 校验、精确 ITOM 用户映射与脱敏审计；P1 已实现普通员工只创建 IT 服务请求或登记 IT 需求，以及实时服务项、动态表单、SLA、流程、派单、预览确认、幂等提交和本人查询；P2 已增加待确认列表、明确工单确认/重开和关闭评价 3 个工具。协议发现不执行工具；任何 `tools/call` 仍必须完整验签、授权并经过 ITOM 领域服务。
...
- **任务与运维总览**：Dashboard 在有权限时展示 Bug、Bug 修复子任务、委派任务和需求开发任务的未完成数量；Bug 所属系统候选直接来自有效 CMDB 配置项。项目进度页在甘特图上方展示取自 WBS 一级任务的阶段进度线；Dashboard 与团队总览的积分排行统一只汇总 `team_contribution` 团队贡献流水并可展开来源明细，`role_result` 岗位结果积分不会再次进入活动积分，避免重复计算。
- **IT 员工分流与跨单据关联（已确认、待实现）**：普通业务员工仍只通过 Aily 创建服务请求或 IT 需求；IT 员工网页将获得轻量“登记 IT 单据”决策向导、六类单据的紧凑说明和案例库。服务请求→事件、服务请求/事件→问题、事件/问题→变更、IT 需求→项目均遵循“创建目标单据 + 审计关联 + 原单据不改类型”的通用 `record_relation` 方案；详见 [`docs/superpowers/specs/2026-07-31-it-staff-intake-and-record-relations-design.md`](docs/superpowers/specs/2026-07-31-it-staff-intake-and-record-relations-design.md)。
...
M45–M46 是冻结版 Helpdesk 历史里程碑，仅保留在 `v1.0.0-feishu-helpdesk`。新开发线依次执行 Aily-MCP P0（移除 Helpdesk、协议/身份/消息）→ P1（真实服务目录、动态表单、服务请求/需求登记、派单）→ P2（解决通知、确认/重开、评价闭环，已通过真实文本和验签按钮 UAT）→ P3（飞书审批，按用户决定暂缓）及 IDC 发布加固（可信 TLS、安全/性能/恢复与真实角色 UAT）。
...
- **Aily + MCP baseline**: P0 implements embedded MCP and identity/audit foundations; P1 implements employee request/requirement intake; P2 adds the pending-confirmation list, explicit-ticket confirmation/reopen, and closed-request rating tools. Discovery cannot execute tools; every `tools/call` still requires full authentication, authorization, own-record scope, and domain validation.
- **IT-staff routing and cross-record relations (approved, not implemented)**: normal employees continue to create only service requests and IT requirements through Aily. IT staff will receive a lightweight “Create IT Record” guide, compact six-record explanations, and a case library. Service request → incident, service request/incident → problem, incident/problem → change, and IT requirement → project follow a generic `record_relation` design: create the target, audit the relation, and never mutate the source type. See the [design baseline](docs/en/superpowers/specs/2026-07-31-it-staff-intake-and-record-relations-design.md).
...
### Milestones
M1–M37 remain the existing platform history. M45–M46 are frozen Helpdesk history only. The new line proceeds through Aily-MCP P0 (remove Helpdesk; protocol, identity, messaging) → P1 (live catalog, dynamic forms, request/requirement intake, dispatch) → P2 (resolution notification, confirmation/reopen, rating loop; live text and signed-button UAT passed) → P3 (Feishu Approval, deferred by user decision) plus IDC release hardening (trusted TLS, security/performance/recovery, and real-role UAT).
