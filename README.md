...
- **任务与运维总览**：Dashboard 在有权限时展示 Bug、Bug 修复子任务、委派任务和需求开发任务的未完成数量；Bug 所属系统候选直接来自有效 CMDB 配置项。项目进度页在甘特图上方展示取自 WBS 一级任务的阶段进度线；积分排行只汇总 `team_contribution` 团队贡献流水并可展开来源明细，`role_result` 岗位结果积分不会再次进入活动积分，避免重复计算。
