...
IDC 是唯一生产交付和最终验收环境。只有用户已确认 `feature-local` 路线，或另行明确批准临时隔离排障时，才允许启动以下仓库 Compose 环境；其中不得包含 IDC 业务数据、生产凭据、Kubernetes Secret、生产 OAuth/Aily 应用、回调或集成配置。本地业务 UAT 只能证明候选版本，不能替代 IDC 正式验收；ngrok 或其他公网隧道仍须另行批准。
...
每个编码任务开始前必须先通过仓库级 [`itom-task-routing`](.agents/skills/itom-task-routing/SKILL.md) skill 确认路线：`production-fix`（IDC 生产修复）、`feature-local`（本地隔离 Docker 新功能候选）或 `code-candidate`（只形成代码候选）。客户端支持时使用可点击选项，否则使用简短编号提问。三条路线分别使用 [`itom-fix-delivery`](.agents/skills/itom-fix-delivery/SKILL.md)、[`itom-feature-delivery`](.agents/skills/itom-feature-delivery/SKILL.md) 和 [`itom-code-candidate-delivery`](.agents/skills/itom-code-candidate-delivery/SKILL.md)。`feature-local` 到达 `local-candidate-ready` 后必须保持容器、数据库、卷、端口和验收数据，直到用户明确完成 UAT 或要求关闭；本地验收通过仍不授权 IDC。`code-candidate` 不得启动、停止或改变任何应用运行环境。`scripts/task-lifecycle.py` 记录分级、范围、验收、候选和分段计时；任何 IDC 写入均须在 CI 通过后另行展示精确标签、对象和回滚方案，并以 `approve-idc` 记录用户明确批准。
...
- **项目组合治理**：项目管理域提供单一主要组合、战略目标与贡献、五维证据评分、人工优先级、准入/暂停/终止治理、跨项目依赖、人员资源超配告警、总体看板、追加式治理审计和不可变版本基线。`project.portfolio_id` 保留兼容，存量关联幂等回填且不伪造历史决策；不包含多组合同时归属、自动排程或 Aily/MCP/网页智能体组合写能力。
...
- **全局表格交互**：主页面统一业务清单的表头始终相对应用内容区吸顶，纵向滚动长清单时仍保持在页面内可见；所有超出内容区宽度的表格使用 Ant Design 自带且保持在当前可视区域底部的唯一横向滚动条，即使尚未滚动到清单最后一行也能访问屏幕外字段。表头和横向滚动条必须共同绑定 `.app-content` 真实滚动容器，系统不再创建第二条全局悬浮滚动条。统一业务清单的每个可见表头分隔线支持鼠标拖拽和键盘左右键微调；鼠标靠近分隔线时显示调整光标，无需常驻醒目手柄，未预设宽度的字段也有默认可调宽度且变化会实际应用到表格布局。布局按当前账号、路由及列集合隔离保存，同页多张表不会相互覆盖。WBS 额外支持前三列冻结，需求总览额外冻结编号与标题两列。
...
### Isolated local candidate and troubleshooting (disabled by default)

IDC is the sole production-delivery and final-acceptance environment. Start the repository Compose stack only for a confirmed `feature-local` route or a separately approved isolated investigation. It must contain no IDC business data, production credentials, Kubernetes Secrets, production OAuth/Aily apps, callbacks, or integrations. Local business UAT proves only a release candidate and never replaces formal IDC acceptance; ngrok or any other public tunnel still requires separate approval.
...
Every coding task first uses the repository-level [`itom-task-routing`](.agents/skills/itom-task-routing/SKILL.md) skill to confirm `production-fix` for an IDC repair, `feature-local` for an isolated local-Docker feature candidate, or `code-candidate` for code-only delivery. Use clickable choices when the client supports them and a concise numbered question otherwise. The routes then follow [`itom-fix-delivery`](.agents/skills/itom-fix-delivery/SKILL.md), [`itom-feature-delivery`](.agents/skills/itom-feature-delivery/SKILL.md), or [`itom-code-candidate-delivery`](.agents/skills/itom-code-candidate-delivery/SKILL.md). After `feature-local` reaches `local-candidate-ready`, its containers, database, volumes, ports, and acceptance data remain available until the user explicitly completes UAT or requests shutdown; local acceptance still does not authorize IDC. `code-candidate` never starts, stops, or alters an application runtime. `scripts/task-lifecycle.py` records grade, scope, acceptance, candidates, and segmented time. Every IDC write requires a separate post-CI presentation of the exact tag, affected objects, and rollback plan plus explicit `approve-idc` authorization.
...
- **Portfolio governance**: Project Management now provides one active primary portfolio, strategic objectives and contributions, five-dimension evidence scoring, a separate human rank, admission/pause/termination decisions, cross-project dependencies, person-allocation conflict warnings, an aggregate dashboard, append-only governance audit, and immutable versioned baselines. `project.portfolio_id` remains compatible and existing relations are idempotently backfilled without invented decisions. Concurrent multi-portfolio membership, automatic scheduling, and Aily/MCP/Web-Agent portfolio writes remain excluded.
...
- **Global table interaction**: shared business-list headers on primary pages stay pinned to the application content viewport while long lists scroll vertically. Every table wider than the content area uses Ant Design's single native, operable horizontal scrollbar at the bottom of the current viewport, so off-screen columns remain reachable before the final row is visible. Both the sticky header and the single scrollbar bind to the real `.app-content` scroll container; ITOM does not create a second global floating scrollbar. Every visible header separator supports mouse dragging and keyboard arrow-key adjustment; the resize cursor appears only near the separator, without a prominent permanent handle. Columns without a predefined width also receive a resizable default and changes take effect in the table layout. Layout is isolated by current account, route, and column set so multiple tables on one page cannot overwrite each other. The WBS table additionally freezes the first three columns; Requirement Overview freezes code and title.
...
### Milestones
M1–M37 remain the existing platform history, and M109 adds portfolio objectives, evidence scoring, human priority, cross-project dependencies, resource-conflict warnings, governance audit, and immutable baselines. M45–M46 are frozen Helpdesk history only. The Aily-MCP line has completed P0 (remove Helpdesk; protocol, identity, messaging), P1 (live catalog, dynamic forms, requester service-request and BDO requirement intake, dispatch), and P2 (resolution notification, confirmation/reopen, rating loop); P3 Feishu Approval is deferred while IDC hardening continues. The later web-agent program uses independent WA0–WA4 identifiers to avoid confusion with Aily phases; WA0 Tasks 1–8 persistence, policy/redaction, secure model gateway, provider/profile administration APIs, owner-scoped web conversations, the generic confirmed L3 action boundary, and guarded POST-SSE orchestration are implemented, while Task 9 acceptance and WA1+ concrete domain capabilities remain pending.
