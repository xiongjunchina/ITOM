...
- **WBS 表格交互**：WBS 宽表支持 Excel 式冻结窗格（表头 + 左侧前三列）、由专用组件维护的一条浏览器原生底部悬浮横向滚动条及表内原生兜底、列宽与行高拖拽调整；“阶段”和“操作”列按最小可用宽度紧凑展示；完成度提供 0%/50%/100% 预设并支持自定义 0–100% 整数；父子任务进度自动级联/汇总；布局按项目保存在当前浏览器。清单支持复选框/全选和选中后出现的批量删除。当前完成度为 100% 的任务禁选并锁定删除、增加子任务和结构调整；经授权把完成度纠正到 100% 以下后，界面重新加载服务端能力并立即恢复该任务允许的操作，同时保留首次完成时间作为审计事实。结构调整仍受权限、引用关系以及已完成同级/后代保护约束，服务器重算顺序和 WBS 编码并保留审计。
...
- **WBS table interaction**: the wide WBS table provides an Excel-style freeze pane (header plus the first three columns), exactly one sticky browser-native bottom horizontal scrollbar with an in-table native fallback, and draggable column widths and row heights. It also provides checkbox/select-all and a contextual Batch Delete toolbar; completed tasks are not selectable. Completion offers 0%/50%/100% presets plus a custom integer from 0–100%; parent/child progress cascades and rolls up automatically; layout is saved per project in the current browser. A confirmed-complete task is locked against deletion and structural change; an unstarted task/subtree may change parent or be drag-sorted among siblings, with the server recalculating order and WBS codes and retaining audit history.
