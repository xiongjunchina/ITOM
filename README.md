...
IDC 仍是唯一生产运行与最终验收环境。只有已确认的 `feature-local` 任务或用户明确批准的临时隔离排障，才允许启动以下本地 Compose 环境；本地候选结果不能替代 IDC 最终验收，也不得连接或复制 IDC 业务数据、凭据、Secret、OAuth/Aily 应用或回调。

```bash
cd deploy && WEB_PORT=18180 docker compose up -d --build
# 前端 http://localhost:18180   API 文档 http://localhost:18180/api/docs
# 管理员口令只从本地 deploy 环境变量 ADMIN_INIT_PASSWORD 注入，不写入文档或命令历史
...
- **全局表格交互**：主页面统一业务清单的表头始终相对应用内容区吸顶，普通清单和 WBS 共用同一横向滚动控制器。统一清单默认冻结前两个业务字段，复选框若存在也保持固定但不计入这两个字段；WBS 继续使用其专用前三列冻结。默认列宽按当前已加载行的真实渲染内容与表头共同测量，数据和表头均保持单行、完整显示；当前列全为空时至少容纳表头。短表按自然宽度紧凑展示，不把剩余视口宽度摊回列内；宽表按可见列宽精确求和并通过原生横向滚动访问。只有当前活动宽表与 `.app-content` 横纵相交时，页面底部才显示一条浏览器原生悬浮横向滚动条；它与始终保留 `overflow-x:auto` 的真实表体双向同步。右固定操作区和左固定字段均使用不透明背景及边界阴影，滚动内容不得透出或覆盖。表头拖拽会同步更新吸顶表头、普通表头、固定列和全部数据行的对应 `colgroup`；用户拖拽过的列优先使用个人宽度，“恢复默认”重新进入内容自适应。操作列按当前结果集最大可见操作数动态压缩到 64–176px并保留悬停说明和无障碍标签。服务目录卡片标题保持单行，元数据省略，操作在底部单行紧凑排列。
- **列表分页与导出**：分页器的页大小为受控状态，统一支持 10/20/50/100 条；服务项等前端本地分页表格切换页大小不会被固定默认值覆盖，服务端分页列表同步传递 `page_size`。远端分页清单始终保留 API 返回的真实过滤后 `total`，不得以当前页加载条数覆盖，因而可继续翻到第 2 页及之后；服务请求和需求总览导出均由服务端导出当前授权范围内的全部筛选结果，而不是当前页。
...
- **WBS 表格交互**：WBS 宽表支持 Excel 式冻结窗格（表头 + 左侧前三列），复用全局唯一底部悬浮横向滚动条及表内原生兜底，同时保留专用列宽与行高拖拽调整；“阶段”和“操作”列按最小可用宽度紧凑展示；完成度提供 0%/50%/100% 预设并支持自定义 0–100% 整数；父子任务进度自动级联/汇总；布局按项目保存在当前浏览器。表格底部可按 50（默认）/100/200/全部控制显示；有限模式提供页码（如 75 行的第 2 页目标为第 51–75 行），回显当前显示数/总数并额外补齐祖先。勾选父项联动全部可操作后代并跨页保留，已完成项保持禁用；拖动未选行移动其子树，拖动已选行按树序原子移动归一后的选中根，可落在目标之前、内部或之后，原调整层级窗口继续保留。批量删除仍逐项执行原校验。填写不晚于今天且不早于实际开始的实际结束会自动完成任务，未来实际结束被拒绝；当前完成度为 100% 的任务锁定实际日期、禁选并锁定删除、增加子任务和结构调整。经授权用一次独立请求把完成度纠正到 100% 以下后，系统清空实际结束、保留首次完成时间并恢复日期及结构操作；同一请求不得同时重开并修改实际日期。结构移动在一个事务内校验权限、项目范围、引用、完成同级/后代、目标与循环关系，任一根无效整批回滚；成功后重算顺序和 WBS 编码并逐根保留审计。
- **受控批量删除**：业务清单可勾选 1–100 条请求批量删除；服务器逐项重用单条删除的授权、状态、引用和审计约束，支持部分成功和逐条拒绝原因。覆盖工单、需求、Bug、三类任务、培训、WBS、CMDB 配置项/关系、服务目录/服务项；不提供级联绕过。
...
- **任务管理（M82）**：开发任务包含“需求开发 / Bug 修复 / 项目开发”，另设“委派任务”。需求开发允许先登记不关联需求、后续再补充关联，Excel 模板与导入同样把关联需求编号作为可选项；项目开发要求关联项目、WBS 可选，用于补充项目 WBS 尚未细拆到开发活动的工作。三类开发任务均显示可排序的任务编号和登记时间：需求开发为 `RT-YYYYMM-NNNN`，项目开发为 `PT-YYYYMM-NNNN`，Bug 修复父单保留 `BG-YYYYMM-NNNN`，每条实际修复子任务使用 `BT-YYYYMM-NNNN`。开发任务清单统一显示“登记人”：网页直接登记取当前登录人员，需求转化取原需求登记人，项目/WBS 转化取项目经理，Bug 修复取 Bug 登记人；历史或系统管理员无人员映射时允许显示为空且仍可从账号审计追溯。项目开发清单以“任务状态”为唯一状态口径，并把追加式进度中的最新有效百分比派生为“完成度”；文字批注只在详情的“进展说明”时间线展示。第一次有效进度会把待处理任务启动为进行中，普通进度最高 99%，只有显式“完成任务”并填写完成说明后才原子写入已完成、100%、完成时间和完成记录；旧进度不迁移、不覆盖。任务指派时通知负责人，负责人更新进度时通知登记人，统一通过站内通知及已启用的飞书 Outbox 出口发送。Bug 支持编辑所属系统、编号进入详情和与流程单据一致的创建人回改/删除窗口，管理员保留全程删除权。启动迁移只为缺少编号的历史需求开发和 Bug 修复子任务按原登记月份及稳定创建顺序补齐 `RT`/`BT`，不覆盖原任务 ID、登记时间、关联关系、状态或软删除数据。所有任务写入继续由服务端 RBAC、记录范围、状态、审计和软删除校验。
...
- **团队总览人员负载**：接口返回全部在岗 IT 成员的未完成工单、项目任务和需求任务负载；“项目任务”统一包含未完成 WBS 工作包与未完成项目开发任务。页面保持按合计降序，并在浏览器本地每页展示 20 人，支持翻页、搜索、排序和导出，工具栏总数与在岗成员范围一致。
- **CMDB 职责与培训活动**：所有配置项必须维护“技术负责人”，负责技术运行与维护；仅“应用”配置项额外维护“产品经理”，负责 Bug 确认和验证关闭，两者可为同一人但不互相替代。新建或编辑应用必须配置产品经理，历史非应用记录的旧值保持可追溯但不在页面展示。CMDB 导入模板将“技术负责人姓名”与“应用产品经理姓名”分列；后者仅应用必填，按在岗人员姓名匹配，不能用技术负责人列代填。CMDB 类别标签按当前主数据代码与显示名统一匹配；“应用”同时兼容历史 `app`、`application` 和“应用”值，因此标签页与总清单一致且不改写存量 CI。培训活动保存登记人；管理员、CIO 或登记人可修改/删除。参与人按真实部门树展示，勾选部门即可级联选择该部门全部 IT 成员，仍可逐人调整。整部门登记会冻结部门 ID、显示名与当时人员范围；清单显示部门名和范围外个别参与人，积分始终按冻结的人员 ID 计算，历史活动不会因后续调岗而重解释。变更主讲或参与人会在当前未发布、未锁定考核期撤销旧培训积分并按当前规则重算；历史/已发布/锁定周期禁止影响积分的改动或删除，非计分资料仍可编辑并审计。
...
### Isolated local development and table UAT (disabled by default)

IDC remains the sole production runtime and final acceptance environment. Start the following Compose stack only for a confirmed `feature-local` task or an explicitly approved isolated investigation. A local candidate never replaces IDC final acceptance, and must not connect to or copy IDC business data, credentials, Secrets, OAuth/Aily applications, or callbacks.

```bash
cd deploy && WEB_PORT=18180 docker compose up -d --build
# Web  http://localhost:18180   API docs http://localhost:18180/api/docs
# Inject the administrator password only through the local ADMIN_INIT_PASSWORD deploy variable; never place it in documentation or shell history.
...
- **Global table interaction**: shared business lists pin their headers and freeze the first two business fields by default; a selection checkbox remains fixed without counting toward those two fields, while WBS retains its specialized first-three-column freeze. Default widths are measured from the rendered header and currently loaded rows, keeping both header and data single-line and fully visible; an all-empty column still fits its header. Short tables retain their compact natural width instead of distributing spare viewport space, while wide tables use the exact visible-width sum and native horizontal access. One browser-native floating scrollbar synchronizes with the real `overflow-x:auto` body, and opaque fixed cells mask scrolled content. A resize updates every header/body `colgroup`; an explicitly resized column keeps its account preference until Restore Defaults returns it to content-driven sizing. Actions remain compact at 64–176px with tooltips and accessible labels. Service-catalog cards keep the title and metadata single-line and place actions in a compact bottom row.
- **Task management**: Requirement Development, Project Development, and Bug Fix lists expose sortable task numbers and registration timestamps. Requirement tasks use `RT-YYYYMM-NNNN`, project tasks use `PT-YYYYMM-NNNN`, Bug parent records keep `BG-YYYYMM-NNNN`, and each actual Bug repair child uses `BT-YYYYMM-NNNN`. They also expose a Registrar column: directly registered work uses the current person; requirement conversion inherits the requirement requester; project/WBS conversion inherits the project manager; Bug Fix uses the Bug reporter. Project Development uses Task Status as the sole state authority and derives a Completion percentage from append-only progress; free-text notes remain in the detail timeline. The first valid progress update starts a pending task, regular progress is capped at 99%, and only the explicit Complete Task action atomically writes Done, 100%, completion time, and the required completion note. Startup migration fills only missing historical `RT`/`BT` codes using the original registration month and deterministic creation order; it does not overwrite IDs, timestamps, relations, states, or soft-delete data. Historical progress rows are not migrated or overwritten. After a Bug record is created, its reporter or another authorized workflow editor can attach screenshots/logs through the guarded generic attachment route; viewers require `task_bug.view`, and failed individual files never roll back the Bug record.
- **List pagination and export**: page-size selectors are controlled state everywhere and consistently support 10/20/50/100 rows; local-paginated tables such as Service Items no longer reset a selected size to a hard-coded default, while server-paginated lists pass the selected `page_size` to the API and retain the server's full filtered `total`. Service Request and Requirement Overview exports are server-side and include every matching record in the current authorized scope rather than only the current page.
...
- **WBS table interaction**: the wide WBS table provides an Excel-style freeze pane (header plus the first three columns), reuses the single global browser-native bottom scrollbar with an in-table native fallback, and retains specialized draggable column widths and row heights. Its bottom control shows 50 rows by default and supports 100, 200, or All. Finite modes add page navigation (for 75 rows, page 2 targets rows 51–75), report shown/total counts, and add required ancestors without consuming page slots. Selecting a parent selects every eligible descendant and persists across pages while completed rows stay disabled. Dragging an unselected row moves its subtree; dragging a selected row atomically moves normalized selected roots in tree order to before, inside, or after a target, with the Adjust Hierarchy dialog retained as a fallback. Batch Delete still applies per-record guards. Completion offers 0%/50%/100% presets plus a custom integer from 0–100%; parent/child progress cascades and rolls up automatically; layout is saved per project in the current browser. A non-future actual end that is not earlier than actual start automatically completes the task. At current progress 100%, actual dates, deletion, and structural changes are locked. A separate correction below 100% reopens the task, clears actual end, keeps the first completion timestamp, and restores permitted date/structure actions; the same request cannot both reopen and change actual dates. Structural moves validate permission, project scope, references, completed siblings/descendants, target layer, and cycles in one transaction; any invalid root rolls back the whole batch, while success recalculates order/WBS codes and audits each root.
- **Controlled batch deletion**: business lists can select 1–100 records for a delete request; the server reuses the single-record authorization, state, reference, and audit constraints for each item, supports partial success, and returns each rejection reason. It covers tickets, requirements, Bugs, the three task types, training, WBS, CMDB CIs/relations, and service catalogs/items; no cascade bypass is provided.
...
- **DMC/project governance alignment (P0)**: requirement evaluation now uses five dimensions (D1 strategic fit, D2 business value, D3 technical feasibility, D4 business maturity combining organization/process readiness, D5 risk). Historical six-dimension records remain readable and use their frozen legacy weights without exposing version labels. Projects can record annual-plan versus out-of-plan source, plan year, authorization amount and decision status; `POST /api/governance/dmc-decisions` records offline DMC/authorization outcomes with audit and evidence references, without introducing online voting or changing the CMDB/Dashboard contract.
...
- `SortableTable` derives default widths from rendered headers/current rows and persists only explicit user overrides through `PATCH /api/auth/me/preferences.table_views.manual_widths`; required identifiers/titles/action columns remain protected. Requirement Overview adds the requester column and keeps the left context available through horizontal scrolling.
...
- **User-controlled synchronization**: inspect the worktree and preserve unrelated local changes before development. Local and GitHub tips do not need to match before coding. Implement and verify the approved local candidate first; commit/push/PR and IDC release remain separate user-approved actions.
