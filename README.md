...
- **全局表格交互**：主页面统一业务清单的表头始终相对应用内容区吸顶，纵向滚动长清单时仍保持在页面内可见；所有超出内容区宽度的表格由 ITOM 为当前活动宽表提供一条浏览器原生、可拖拽的底部悬浮横向滚动条，即使尚未滚动到清单最后一行也能访问屏幕外字段。该滚动条与表格正文双向同步；只有它已完成尺寸测量且可用时才隐藏表内重复条，测量或浏览器异常时表内原生横向滚动条仍可操作。表头定位仍绑定 `.app-content` 真实滚动容器。统一业务清单的每个可见表头分隔线支持鼠标拖拽和键盘左右键微调；鼠标靠近分隔线时显示调整光标，无需常驻醒目手柄，未预设宽度的字段也有默认可调宽度且变化会实际应用到表格布局。布局按当前账号、路由及列集合隔离保存，同页多张表不会相互覆盖。操作列统一为带悬停说明和无障碍标签的紧凑图标，并忽略历史保存的过宽操作列偏好，以最小可用宽度和间距展示；勾选记录后，顶部才显示当前页面和所选记录允许的文字批量操作按钮。WBS 额外支持前三列冻结，需求总览额外冻结编号与标题两列。
...
- **Global table interaction**: shared business-list headers on primary pages stay pinned to the application content viewport while long lists scroll vertically. Every table wider than the content area gives the current active wide table one browser-native, draggable floating horizontal scrollbar at the bottom of the viewport, so off-screen columns remain reachable before the final row is visible. It synchronizes in both directions with the table body; the duplicate in-table scrollbar is hidden only after the floating control has been measured and is usable, and remains an operable fallback if measurement or the browser fails. Header positioning still binds to the real `.app-content` scroll container. Every visible header separator supports mouse dragging and keyboard arrow-key adjustment; the resize cursor appears only near the separator, without a prominent permanent handle. Columns without a predefined width also receive a resizable default and changes take effect in the table layout. Layout is isolated by current account, route, and column set so multiple tables on one page cannot overwrite each other. Action columns use compact icon-only controls with hover Tooltips and accessible labels; after rows are selected, the list top shows only the explicit-text batch actions valid for that page and selection. The WBS table additionally freezes the first three columns; Requirement Overview freezes code and title.
