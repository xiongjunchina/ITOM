...
- **任务与运维总览**：Dashboard 在有权限时展示 Bug、Bug 修复子任务、委派任务和需求开发任务的未完成数量；Bug 所属系统候选直接来自有效 CMDB 配置项。项目进度页在甘特图上方展示取自 WBS 一级任务的阶段进度线；积分排行保留积分台账原始正负汇总并可展开来源明细，避免把原始排行与人效归一化结果混为一谈。
