#!/usr/bin/env python3
"""Local lifecycle ledger and gates for ITOM corrective tasks."""
...
def start(args: argparse.Namespace, ledger: Ledger) -> None:
    ...
    timestamp = now()
    data = {
        "schema": 1, "id": args.id, "grade": args.grade, "scope": args.scope,
    ...
    ledger.save(data)
    print(f"Started {args.id}: grade={args.grade}, scope={args.scope}")
...
def mutate(args: argparse.Namespace, ledger: Ledger) -> None:
    ...
    if cmd == "regrade":
        ...
        data.update(root_cause_required=False, status="diagnosing", target_verified_at=None,
                    docs_assessed_at=None, candidate_frozen_at=None, ci_attempts=0, idc_attempts=0)
...
def report(ledger: Ledger) -> None:
    ...
    output = {"id": data["id"], "status": data["status"], "grade": data["grade"],
...
def close(args: argparse.Namespace, ledger: Ledger) -> None:
    data = ledger.load()
    require(data, args.outcome == "blocked" or data["status"] in {"ci_passed", "idc_accepted"},
            "Only a passed or explicitly blocked task can be closed.")
...
def parser() -> argparse.ArgumentParser:
    ...
    p = sub.add_parser("start"); p.add_argument("--id", required=True); p.add_argument("--grade", choices="SML", required=True); p.add_argument("--scope", required=True); p.add_argument("--acceptance", required=True)
    ...
    p = sub.add_parser("close"); p.add_argument("--outcome", choices=("complete", "blocked"), required=True)
    return result
